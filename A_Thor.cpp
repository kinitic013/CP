...
void solve(ll t)
{
    ...
    REP(i,0,q)
    {
        ...
        {
            val = max(0LL,val - start);
            while(val--)
            {
                ll diff = total[notifications[start]] - unread[notifications[start]];
                counted[notifications[start]]++;
                if(counted[notifications[start]] > diff)// not read
                {
                    unread[notifications[start]]--;
                    sum--;
                    //
                }
                start++;
