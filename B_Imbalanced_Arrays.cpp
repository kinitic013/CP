...
void solve(ll t)
{
    ...
    while(i<=last)
    {
        ...
        //     return;
        // }
        if(arr[i][1] == count_greater)
        {
            arr[i][2] = -1*curr;
            curr--;
        }
        else if(arr[i][1] > count_greater)
        {
            arr[last][2] = curr;
            count_greater++;
            curr--;
    ...
    REP(i,0,n) checker_array.push_back(arr[i][2]);
    REP(i,0,n)
    {
        auto it = upper_bound(all(checker_array),-1*checker_array[i]);
        ll count = n - (it - checker_array.begin());
        if(count != arr[i][1])
        {
            cout<<"NO";
            return;
