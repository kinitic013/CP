...
void solve(vi& p)
{
    ...
    ll AA = *min_element(all(a));
    ll BB = *min_element(all(b));
    ll B = *max_element(all(b));
    ll count = 0;
    bool flag = 0;
    ...
    {
        count += abs(a[i]-b[i]);
        if(b[n] <= max<ll>(a[i],b[i]) && b[n]>=min<ll>(a[i],b[i]))
        {
            flag = 1;
        }
        val = min<ll>(val , abs(a[i] - b[n]));
        val = min<ll>(val , abs(b[i] - b[n]));
    }
    count++;
    if(!flag)
    {
