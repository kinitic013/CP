...
ll query(ll l, ll r) {
    if (l > r) return -1;
    ll size = r - l + 1;
    ll range = floor(log2(size));
    return (sparse[l][range] & sparse[r - (1 << range) + 1][range]);
}
void solve(ll t)
{
    ll n,q;cin>>n>>q;
    vvi queries(q,vi(3,0));
    vvi arr(n,vi(30,0));
    REP(i,0,q)
    {
        ...
        while(val)
        {
            if(val&1)
            {
                arr[queries[i][0]][bit] += 1;
                if(queries[i][1] + 1 < n)
                arr[queries[i][1] + 1][bit] -= 1;
            }
            val/=2;
            bit++;
        }
    ...
    // vector<vector<ll>> sparse;
    {
        ...
        sparse = vvi(n, vi(log_n, 0));
        
        // Initialize sparse table with array values
        for(ll i = 0; i < n; i++) {
            sparse[i][0] = v[i];
        }
