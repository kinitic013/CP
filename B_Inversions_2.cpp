...
ll query(ll start,ll end , ll index , ll target)
{
    ll left = index*2 + 1;
    ll right = index*2 + 2;
    ll mid = (start + end )/2;
    ...
    if(start == end)
    {
        if(segTree[index] == target)
        return start;
        else
        return -1;
    }
    if(target <= segTree[left])
    {
        return query(start,mid,left,target);
    }
    else
    {
        return query(mid+1,end,right,target-segTree[left]);
    }
}
void solve(ll t)
{
    ...
    build(visited,0,n-1,0);
    vi res;
    for(int i=n-1;i>=0;i--)
    {
        ll val = query(0,n-1,0,i+1-arr[i]);
        res.push_back(val);
        update(visited,0,n-1,0,val,0);
    }
    reverse(all(res));
    for(auto x : res)
    cout<<x+1<<" ";
