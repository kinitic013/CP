...
void intersection(pair<double,double>& range,pair<double,double>& curr_range,bool & res)
{
    if(range.ff>curr_range.ss || curr_range.ff > range.ss )
    {
        res = 0;
        return;
    }
    range = {max<double>(range.ff,curr_range.ff),min<double>(range.ss,curr_range.ss)};
    return;
}
bool checker(vpii& arr,double& ans , double& time )
{
    ll n = arr.size();
    ll curr = 0;
    pair<double,double> range = {-2*(1e8),2*(1e8)};
    REP(i,0,n)
    {
        pair<double,double> curr_range = {arr[i].ff +  arr[i].ss - time , arr[i].ff + time - arr[i].ss};
        bool res = true;
        intersection(range,curr_range,res);
        if(!res)
        return res;
    }
    ans = (range.ff + range.ss)/2;
    return true;
}
void solve()
{
    ...
    while(counter < 100)
    {
        ...
        double mid = (l + h )/2;
        double res = 0;
        bool check = checker(arr,res,mid);
        if(check)
        {
            h = mid;
            ans = res;
