...
std::pair<long long, long long> unset_msb(long long num) {
    if (num == 0) return {0, 0};
    
    // Find the position of the MSB
    int msb_pos = sizeof(long long) * CHAR_BIT - 1;
    while (msb_pos > 0 && !(num & (1LL << msb_pos))) {
        msb_pos--;
    }
    
    // Calculate the value of the MSB
    long long msb_value = 1LL << msb_pos;
    
    // Unset the MSB
    long long result = num & ~msb_value;
    
    return {result, msb_value};
}
void solve(ll t)
{
    ll n,k;cin>>n>>k;
    ll less = unset_msb(k).ff;
    ll msb = unset_msb(k).ss;
    vi res;
    ll ele = 1;
    while(ele<=n)
    {
