...
void solve()
{
    ...
    cout<<"YES";
    return;
    // vector<pair<ll,pii>> dist(n);

    // vi m;
    // REP(i,0,n)
    // {
    //     ll time_needed = (abs(arr[i].ss)/x) + ((abs(arr[i].ss)%x)&1);
    //     dist[i].ff = time_needed;
    //     dist[i].ss.ss = arr[i].ss;
    //     dist[i].ss.ff = arr[i].ff;
    // }
    // sort(all(dist),cmps);
    // ll time = 0;
    // ll i = 0;
    ...
    //     while(k>0 && i<n)
    //     {
    //         ll temp = min<ll>(k,dist[i].ss.ff);
    //         k -= temp;
    //         dist[i].ss.ff -= temp;
    //         if(dist[i].ss.ff == 0)
    //         {
    //             if(time>abs(dist[i].ss.ss))
    //             {
    //                 cout<<"NO";
    //                 return;
    //             }
