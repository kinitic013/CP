...
void build(vector<long long>& arr, long long start , long long end , long long index)
{
    ...
    segTree[index] = (segTree[left]+segTree[right])%MOD;
}
void rangeUpdate(vector<long long>& arr, long long start, long long end, long long index, long long l, long long r, long long value) {
    ...
    if (lazySegTree[index] != 1) {
        segTree[index] =( segTree[index]*lazySegTree[index])%MOD;
        if (start != end) { // Propagate lazy value to children
            lazySegTree[left] = (lazySegTree[left] * lazySegTree[index])%MOD;
            lazySegTree[right] = (lazySegTree[right]*  lazySegTree[index])%MOD;
        }
        lazySegTree[index] = 1;
    }
    ...
    if (start >= l && end <= r) {
        ...
        if (start != end) { // Update lazy for children
            lazySegTree[left] =  (lazySegTree[left] * value)%MOD;
            lazySegTree[right] = (lazySegTree[right] * value)%MOD;
        }
        return;
    }
    ...
    segTree[index] = (segTree[left]+ segTree[right])%MOD; // Update the current node
}
...
void solve(ll t)
{
    ...
    REP(i,0,q)
    {
        ...
        {
            cin>>value;
            rangeUpdate(arr,0,n-1,0,l,r,value);
        }
        else
        {
            cout<<queryRange(0,n-1,0,l,r)<<endl;
        }
    }
}
int main()
