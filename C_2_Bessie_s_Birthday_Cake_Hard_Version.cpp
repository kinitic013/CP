...
void solve(vi& p)
{
    ...
    REP(i,1,x)
    {
        ...
        if(arr[i] - arr[i-1] > 1)
        {
            ...
            {
                diffodd.push_back(arr[i] - arr[i-1] - 1);
            }
            else
            {
                diffeven.push_back(arr[i] - arr[i-1] - 1);
            }
        }
    }
    ll difference = arr[0] - 1 + n - arr[x-1]; 
    if(difference)
    {
        if(difference&1)
        diffodd.pb(difference);
        else
        diffeven.pb(difference);
    }
    ll count = max<ll>(0,x-2);
    int i = 0;
    sort(all(diffodd));
    while(i<diffodd.size() && y>0)
    {
        if(diffodd[i] == 1)
        {
            count++;
    ...
    i = 0;
    while(i<diffeven.size() && y>0)
    {
        ll limit = (diffeven[i]/2) ;
        if(y < limit)
        {
            count += 2*y;
            y -= 2*y;
