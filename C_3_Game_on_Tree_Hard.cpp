...
void dfs_up(vvi& adj,vi&ans,vi&up,vi&down,ll parent,ll node)
{
    vi count = {0,0};
    for(auto x : adj[node])
    {
    ...
    for(auto x : adj[node])
    {
        if(x != parent)
        {
            ...
            // if there is any one then that will be choosen
            if(count[1] != 0)
            up[x] = 0;
            else
            up[x] = 1;
...
void solve(vi& p)
{
    ...
    dfs_down(adj,down,0,1);
    up[1] = -1;
    dfs_up(adj,ans,up,down,0,1);
    ans[1] = down[1];
    debug(up,down,ans);
    REP(i,0,t)
    {
