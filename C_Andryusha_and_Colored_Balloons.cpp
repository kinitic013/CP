...
void dfs(vvi & adj,vi& color , ll max_limit,ll node,ll parent)
{
    ll curr = 1;
    for(auto x : adj[node])
    {
        if(x != parent)
        {
            while(color[node] == curr || color[parent] == curr)
            {
                curr++;
                if(curr > max_limit)
                curr = 1;
    ...
    for(auto x : adj[node])
    {
        if(x != parent)
        {
            dfs(adj,color,max_limit ,x,node);
        }
    }
}
void solve(ll t)
{
    ...
    REP(i,0,n-1)
    {
        ...
        degree[u]++;
        degree[v]++;
        max_degree = max(max_degree,max(degree[u],degree[v]));
    }
    ll limit = max_degree+1;
    ll start = 1;
    REP(i,1,n+1)
    {
        if(degree[i] == 1)
    ...
    dfs(adj,color,limit,adj[start][0],start);
    cout<<limit<<"\n";
    REP(i,1,n+1)
    cout<<color[i]<<" ";
    return;
