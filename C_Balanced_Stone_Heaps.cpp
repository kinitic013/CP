...
//Great Cheaker Funciton

bool checker(vi& temp , ll limit)
{
    vi arr = temp;
    ll n = arr.size();
    vi extra(n,0);
    // extra[n-2] += (arr[n-1] - limit)/2;
    // extra[n-3] += 2*((arr[n-1] - limit)/3);
    // arr[n-1] -= 3*((arr[n-1] - limit)/3);

    for(int i=n-1;i>=2;i--)
    {
        ll diff =max<ll>(0, arr[i] - max<ll>(0,(limit - extra[i])));
        ll d = diff/3;
        extra[i-1] +=d;
        extra[i-2] += 2*d;
        arr[i] -=3*d;
