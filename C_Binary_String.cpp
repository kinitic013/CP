...
namespace _DEBUG_UTIL_
{
    ...
    void print(T (&arr)[N])
    {
        /* Helps in iterating through arrays and arrvent decays */
        int f = 0;
        cerr << '{';
        for (auto &i : arr)
            cerr << (f++ ? "," : ""), print(i);
    ...
    void print(T (&mat)[N][M])
    {
        /* Helps in iterating through 2D arrays and arrvent decays and also print arrays in new line */
        int f = 0;
        cerr << "{\n";
        for (auto &i : mat)
            cerr << (f++ ? ",\n" : ""), print(i);
...
bool checker(vi& arr,vi& pre, ll limit)
{
    ll n = arr.size();
    ll i = 0;
    ll j = i;
    ...
    while(j<n)
    {
        curr_zero = pre[j]-pre[i];

        curr_one = j-i+1;
        removed_one = n - curr_one;
        ll ans = max<ll>(removed_one,curr_zero);
        if(ans<=limit)
        return true;
...
void solve(ll t)
{
    ...
    while(counter<40)
    {
        counter++;
        ll mid = (l+h)/2;
        bool check  = checker(arr,pre,mid);
        if(check)
        {
            h = mid;
        }
