...
void solve(ll t)
{
    ...
    cin>>b.ff>>b.ss;
    vpii dist(n);
    ll path = sqr(a.ff-b.ff) + sqr(a.ss-b.ss);
    ll minia=LONG_LONG_MAX,minib=LONG_LONG_MAX;
    REP(i,0,n)
    {
        ll dista = sqr(arr[i].ff-a.ff) + sqr(arr[i].ss - a.ss);
        ll distb = sqr(arr[i].ff-b.ff) + sqr(arr[i].ss - b.ss);
        dist[i] = {dista,distb};
        minia = min(minia,dista);
        minib = min(minib,distb);
    }
    if(minia == 0)
    {
        cout<<"NO";return;
