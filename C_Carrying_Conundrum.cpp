...
void solve()
{
    ...
    for(int i=n-1;i>=0;i--)
    {
        ll total = range(arr[i] - '0' );
        ll carry = range(arr[i] - '0' -1);

        // without carry 
        dp[i].ff  = (total*dp[i+2].ff) + (carry*dp[i+2].ss);
        // with carry forward
        total = range(arr[i] - '0'  + 10);
        carry = range(arr[i] - '0'  + 9);
        dp[i].ss = (total * dp[i+2].ff) + (carry*dp[i+2].ss);
        debug(dp[i]);
    }
    cout<<(dp[0].ff*dp[1].ff) -2;
    return;
}
int main()
{
