...
void solve(vi& p)
{
    ...
    for(int i = 1;i<n;i+=2)
    {
        ll prev = 0;
        if(i-1>=0)
        prev = arr[i-1] + mini[i-1] ;
        ll forw = 0;
        if(i+1<n)
        forw = arr[i+1] + mini[i+1] ;
        ll val =  0 - arr[i];
        need.pb({val,i});
    }
    sort(all(need));
    ...
    REP(i,1,n-1)
    {
        if(arr[i]+mini[i] > arr[i-1]+mini[i-1] && arr[i]+mini[i] > arr[i+1]+mini[i+1])
        countMini++;
    }
    ...
    sort(all(needMaxi));
    ele = (n/2) + 1;
    REP(i,0,needMaxi.size())
    {
        maxi[needMaxi[i].ss] = ele;
        ele++;
    }
    ll countMaxi = 0;
    debug(maxi,arr);
    REP(i,1,n-1)
    {
        if(arr[i]+maxi[i] > arr[i-1]+maxi[i-1] && arr[i]+maxi[i] > arr[i+1]+maxi[i+1])
        countMaxi++;
    }
    // cout<<(countMaxi)<<countMini<<endl;
    if(countMaxi>countMini)
    {
        for(auto x : maxi)
        cout<<x<<" ";
        return;
