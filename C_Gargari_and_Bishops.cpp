...
void solve(vi& p)
{
    ...
    vvi arr(n,vi(n,0));
    vvi dp(n,vi(n,0));
    vi rightDiagonal(2*n + 1 , 0);
    vi leftDiagonal(2*n + 1 , 0);
    REP(i,0,n)
    {
        REP(j,0,n)
        {
            cin>>arr[i][j];
            ll rightIdx = i - j + n;
            rightDiagonal[rightIdx] += arr[i][j];
            ll leftIdx = i + j;
            leftDiagonal[leftIdx] += arr[i][j];
        }
    }
    debug(leftDiagonal,rightDiagonal);
    REP(i,0,n)
    {
        for(int j = n-1;j>=0;j--)
        {
            ll rightIdx = i - j + n;
            ll leftIdx = i + j;            
            dp[i][j] = rightDiagonal[rightIdx] + leftDiagonal[leftIdx] - arr[i][j];
        }
    }
    debug(dp);
    ll black = 0;
    ...
    REP(i,0,n)
    {
        REP(j,0,n)
        {
            ...
            if(sum &1) // black
            {
                if(dp[i][j] >= black)
                bIdx = {i,j};
                black = max<ll>(dp[i][j],black);
            }
            else
            {
                if(dp[i][j] >= white)
                wIdx = {i,j};
                white = max<ll>(dp[i][j] , white);
            }
        }
    }
    cout<<black + white<<endl;
    cout<<bIdx.ff+1<<" "<<bIdx.ss+1<<" "<<wIdx.ff+1<<" "<<wIdx.ss+1<<endl;
    return ;
}
int main()
{
