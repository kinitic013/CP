...
map<char,ll> m;
ll compute(vvi& dp,vector<string>& arr,ll current_string_index,ll current_ending_pointer)
{
    if(current_string_index == -1)
    return 0;

    
    REP(i,0,arr[current_string_index].size())
    {
...
void solve(ll t)
{
    ...
    }
    {
        ...
        REP(i,0,len)
        {
            if(name.find(arr[0][i]) != string::npos )
            {
                if(m[arr[0][i]] == pointer)
                {
                    pointer = (pointer+1)%5;
                    score++;
                    continue;
    ...
    REP(i,1,n)
    {
        REP(start_pointer,0,5)
        {
            dp[i][start_pointer] = max(dp[i-1][start_pointer],dp[i][start_pointer]);
        }
        REP(start_pointer,0,5)
        {
            ll score = 0;
            ll pointer = start_pointer;
            REP(k,0,len)
            {
                if(name.find(arr[i][k]) != string::npos )
                {
                    if(m[arr[i][k]] == pointer)
                    {
                        pointer = (pointer+1)%5;
                        score++;
                        continue;
            ...
            ll val = (score + dp[i-1][start_pointer] );
            dp[i][pointer] = max(dp[i][pointer],val);
            if(dp[i][pointer] < (-1)*pointer)
            dp[i][pointer] = (-1)*pointer;
        }
    }
    ll ans = 0;
    REP(i,0,5)
