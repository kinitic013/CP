...
bool check(string& arr,vi& pre , ll l , ll r)
{
    ll n = arr.size();
    ll val = query(0,n,0,l+1,r);
    if(val < pre[l] )
    return false;
    else
    return true;
...
void solve(ll t)
{
    ...
    debug(pre);
    segTree.resize(4*(n+1),LONG_LONG_MAX);
    build(pre,0,n,0);
    ll l = 0;
    ll r = n/2 + 1;
    while(r-l>1)
    {
        ...
        REP(i,0,n)
        {
            if(i + mid -1 < n)
            {
                if(arr[i] == ')'|| pre[i + mid ] != pre[i])
                {
                    continue;
                }
                bool checker = check(arr,pre,i+1,i + mid -1);
                flag = checker || flag;
            }  
            else
            break; 
    ...
    else
    {
        ...
        REP(i,0,n)
        {
            if(i + l -1 < n)
            {
                if(arr[i] == ')'|| pre[i + l ] != pre[i])
                {
                    continue;
                }
                bool checker = check(arr,pre,i+1,i + l -1);
                if(checker)
                count++;
            }   
        }
