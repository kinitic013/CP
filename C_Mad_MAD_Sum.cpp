...
{ 
    ...
    REP(i,1,b.size())
    {
        ll last = count.size() - 1;
        if(b[i] == count[last].ff)
        count[last].ss++;
        else
        count.pb({b[i],1});
    }
    ll s = 0;
    vi post(count.size(),0);
    ...
    REP(i,0,count.size())
    {
        ...
        ll multi = (post[i] - count[i].ss) + 1;
        ll val1 = (multi * ( count[i].ff * count[i].ss));
        ll val2  = count[i].ff*(((count[i].ss*(count[i].ss-1)))/2);
        ans +=  val1 + val2 ;
    }
    cout<<ans + sum;
}
