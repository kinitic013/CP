...
void solve(ll t)
{
    ...
    {
        if(arr[i].ff <= l)
        dp[1][i] = arr[i].ff - arr[i].ss;

        if(i==0)
        pre[i] = dp[1][i];
        else
        {
            pre[i] = min(pre[i-1],dp[1][i]);
        }
    }
    debug(pre);
    REP(i,2,n+1)
    {
        ...
        REP(j,i-1,n)
        {
            ll ans = pre[j-1] + arr[j].ff + arr[j].ss;
            if(ans <= l)
            {
                dp[i][j] = pre[j-1] + arr[j].ff;
            }
            curr[j] = min<ll>(curr[j-1],dp[i][j]);
        }
        pre = curr;
    }
    for(int i=n;i>=1;i--)
    {
        if(*min_element(all(dp[i])) != inf)
        {
            cout<<i;
            return;
        }
