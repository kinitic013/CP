...
void solve(vi& p)
{
    ...
    REP(i,1,n)
    {
        if(arr[i] != arr[i-1])// barricade is present
        {
            count += i*(n-i);
        }
    }
    REP(j,0,q)
    {
        ...
        cin>>pos>>val;
        ll i = pos - 1;
        // deleting is any barricade disappeared
        if(i+1 < n && arr[i] != arr[i+1])//barricade being disappeared
        {
            count -= (i+1)*(n-(i+1));
        }
        ...
        arr[pos-1] = val;
        if(i+1 < n && arr[i+1] != arr[i])// barricade created
        {
            count += (i+1)*(n-(i+1));
        }
