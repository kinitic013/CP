...
void dfs(vvi& adj,ll node,ll parent,priority_queue <int, vector<int>, greater<int>>& pq,vb& state)
{
    bool curr = state[node];
    for(auto x : adj[node])
    {
        if(x != parent)
        {
            curr = curr && state[x];
            // dfs(adj,x,node,pq,state);
        }
    }
    if(curr)
    {
