...
void solve(ll t)
{
    ...
    reverse(all(arr));
    int n = arr.size();
    vector<vector<vector<ll>>> dp(n,vector<vector<ll>>(5,vi(2,neg_inf)));
    REP(i,0,5)
    {
        if(arr[0]-'A' == i)
        dp[0][i][0] = f[i];
    ...
    REP(i,0,n-1)
    {
        REP(j,0,5)
        {
            ll x = arr[i+1] - 'A';
            {
                ...
                ll val = dp[i][j][0] + extra;
                dp[i+1][max(j,x)][0] = max(dp[i+1][max(j,x)][0],val);
            }


            // state 1
            // changing next element to (0->4)
            {
                REP(k,0,5)// changing next elment to k
                {
                    ll extra = f[k];
                    if(k<j)
                    extra *= -1;
                    ...
                    ll val =  dp[i][j][0] + extra;
                    dp[i+1][max(j,k)][1] = max(dp[i+1][max(j,k)][1],val);
                }
            }
            // change operation already taken place
            {
                ll extra = f[x];
                if(x<j)
                extra*=-1;

                ll val = dp[i][j][1] + extra;
                dp[i+1][max(j,x)][1] = max(dp[i+1][max(j,x)][1],val);
            }
        }
    }
    // // debug(dp);
    ...
    //         // convert current element to j
    //         ll val1 = neg_inf;
    //         REP(k,0,j+1)
    //         {
    //             ll curr = dp[i-1][k][0] + f[j];
    //             val1 = max(val1,curr);
    //         }
    //         // already used one operation
    //         if(arr[i]-'A' == j)
    //         {
    //             REP(k,0,j+1)
    //             {
    //                 ll extra = f[j];
    //                 ll curr = dp[i-1][k][1] + extra;
    //                 val1 = max(val1,curr);
    //             }
    //             dp[i][j][1] = val1;
    //         }
