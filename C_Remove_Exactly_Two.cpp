...
void dfs(vvi&adj,vi& first,vi& second,vi& child_count,ll node,ll parent,ll& res)
{
    ll child = 0;
    for(auto x : adj[node])
    {
        ...
        {
            child++;
            dfs(adj,first,second,child_count,x,node,res);
            first[node] = max(first[node] , first[x]); // looking at child of x
            first[node] = max(first[node] , child_count[x]); // looking at x 
            // second[node] = max(second[node] , second[x]); // looking at answer form 2 removals of childs
        }
    }
    child_count[node] = child;
    // removing current element at second removal
    ll extra = 0;
    if(parent != 0)
    extra = 1;
    for(auto x : adj[node])
    {
        if(x != parent)
        {
            ...
            if(first[x] != -1)
            {
                val1 = first[x] + child + extra;
            }
            val2 = child_count[x] + child - 1 + extra;
            second[node] = max(second[node] , max( val1, val2));
        }
    }
    if(child >=2)
    {
        ...
        for(auto x : adj[node])
        {
            if(x != parent)
            {
                second_maxi = max(second_maxi , max(first[x] , child_count[x]));
                if(second_maxi > maxi)
                swap(second_maxi,maxi);
            }
        }
        res = max(res , maxi + second_maxi + 1);
    }
}
void solve(ll t)
{
    ...
    vi first(n+1,-1);
    vi second(n+1,0);
    vi child_count(n+1,0);
    dfs(adj,first,second,child_count,1,0,res);
    debug(child_count);
    debug(first);
    debug(second);

    REP(i,1,n+1)
    res = max(res , second[i]);
