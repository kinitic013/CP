...
vvi segTree;
void build(vector<ll>& arr, ll start , ll end , ll index , ll k)
{
    if(start == end)
    {
        segTree[index][k] = 0;
    ...
    ll left = index*2+1;
    ll right = index*2 + 2;
    build(arr,start,mid,left,k);
    build(arr,mid+1,end,right,k);
    segTree[index][k] = (segTree[left][k]+segTree[right][k]);
}
void update(vvi& arr , ll start , ll end , ll index , ll pos ,ll value, ll k)
{
    if(start == end)
    {
        arr[pos][k] = value;
        segTree[index][k] = value;
        return;
    }
    ll mid = (start + end)/2;
    ll left = index*2 + 1;
    ...
    if(pos<=mid)
    {
        update(arr,start,mid,left,pos,value,k);
    }
    else
    {
        update(arr,mid+1,end,right,pos,value,k);
    }
    segTree[index][k] = (segTree[left][k]+segTree[right][k]);
}
ll query(ll start,ll end , ll index , ll l , ll r , ll k)
{
    ll left = index*2 + 1;
    ll right = index*2 + 2;
    ll mid = (start + end )/2;
    ...
    {
        ll leftValue = query(start,mid,left,l,r,k);
        ll rightValue = query(mid+1,end,right,l,r,k);
...
void solve(ll t)
{
    ...
    REP(i,0,4*n)
    {
        segTree[i].resize(k+2,0);
    }
    REP(i,0,n)
    {
        ll ele;cin>>ele;
        ele--;
        update(arr,0,n-1,0,ele,1,1);
        REP(j,2,k+2)
        {
            ll val = query(0,n-1,0,0,ele-1,j-1);
            update(arr,0,n-1,0,ele,val,j);
        }
    }
    ll sum = 0;
    for(auto x : arr)
