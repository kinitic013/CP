...
void solve()
{
    ...
    ll extra  = 0;
    ll spreadingFactor = count;
    ll remaining = 0;
    vi remain;
    REP(i,0,n+1)
    {
        extra += max<ll>(0,arr[i] - spreadingFactor);
        if(arr[i] - spreadingFactor > 0)
        {
            remaining++;
            remain.pb(arr[i] - spreadingFactor);
        }
    ...
    ll h=1000000001;
    ll counter = 40;
    sort(all(remain),greater<ll>());
    while(counter--)
    {
        ll mid = (l+h)/2;
        bool check = checker(remain,mid);
        if(check)
        h=mid;
        else
        l=mid;
