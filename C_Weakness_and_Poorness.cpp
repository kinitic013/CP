...
double maxAbsoluteSubarraySum(vector<ll> &arr , double x)
{
    ll n =arr.size();
    double ans = LONG_LONG_MIN;
    double max_so_far = INT_MIN, max_ending_here = 0;
    for (int i = 0; i < n; i++) {
        max_ending_here = max_ending_here +( arr[i] - x);
    ...
    ans = max<double>(ans , max_so_far);

    max_so_far = INT_MIN, max_ending_here = 0;
    for (int i = 0; i < n; i++) {
        max_ending_here = max_ending_here + (x - arr[i]);
        if (max_so_far < max_ending_here)
            max_so_far = max_ending_here;
...
double f(vi&arr , double x)
{
    double ans = maxAbsoluteSubarraySum(arr ,x);
    return ans;
}
double ternary_search(vi& arr, double l, double r) {
    ll counter = 800;
    double eps = 0.000000000001;              //set the error limit here
    while (r - l > eps && counter-->0 ) {
        counter--;
        double m1 = l + (r - l) / 3;
        double m2 = r - (r - l) / 3;
        double f1 = f(arr ,m1);      //evaluates the function at m1
        double f2 = f(arr ,m2);      //evaluates the function at m2
        if (f1 > f2)
            l = m1;
        else
            r = m2;
    }
    return f(arr ,l);                    //return the minimum of f(x) in [l, r]
}
void solve(vi& p)
{
    ll n;cin>>n;
    vi arr(n);
    REP(i,0,n) cin>>arr[i];
    double ans = ternary_search(arr , -10000 , 10000);
    std::cout << std::fixed;
