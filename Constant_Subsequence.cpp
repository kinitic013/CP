...
void solve(ll t)
{
    ...
    ll neg_sum = 0;
    ll pos_sum = 0;
    neg_sum = accumulate(all(neg),0LL);
    pos_sum = accumulate(all(pos),0LL);
    if(pos.size() == 0 || pos.size() == n)
    {
        cout<<max(0LL,neg_sum + pos_sum);
        return;
    }
    ll l = *max_element(all(pos))-1;
    ll r = pos_sum;
    vi res;
    while(r-l>1)
    {
        ...
        {
            while(po < pos.size()  && curr + pos[po] <= mid)
            {
                temp.push_back(pos[po]);
                curr += pos[po];
                po++;
            }
            ll last = curr;
            curr = 0;
            while(ne < neg.size()  &&  curr < last)
            {
                temp.push_back(neg[ne]);
                curr += neg[ne];
                ne++;   
            } 
        }
