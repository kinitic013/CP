...
pii max_depth_node_dfs(vvi& adj,ll parent,ll node,ll depth)
{
    pii ans = {node,depth};
    for(auto x : adj[node])
    {
        if(x != parent)
        {
            pii curr = max_depth_node_dfs(adj,node,x,depth+1);
            if(curr.ss > ans.ss)
            ans = curr;
        }
    }
...
void depthCal(vvi& adj,vi& dep,ll parent,ll node,ll depth)
{
    dep[node] = max(dep[node],depth);
    for(auto x : adj[node])
    {
        if(x != parent)
        {
            depthCal(adj,dep,node,x,depth+1);
        }
    }
}
void solve(vi& p)
{
    ...
    ll u = max_depth_node_dfs(adj,0,1,0).ff;
    ll v = max_depth_node_dfs(adj,0,u,0).ff;
    debug(u,v);
    vi depth(n+1,0);
    depthCal(adj,depth,0,u,0);
    depthCal(adj,depth,0,v,0);
    depth[0]=LONG_LONG_MAX;
    sort(all(depth));
    debug(depth);
    vi res(n+1,0);
    REP(i,1,n+1)
    {
        if(depth[depth.size()-2] < i)
        {
            res[i] = n;
            continue;
        }
        auto it = upper_bound(all(depth),i-1);
        ll curr = 0;
        if(it != depth.begin())
        {
            it--;
            ll count = it - depth.begin() + 1;
            res[i] = count+1;
            debug(i,res[i]);
        }
        else
