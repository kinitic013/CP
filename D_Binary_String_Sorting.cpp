...
// the reason for 1e12 and 1e12 + 1 so that there won't an equivalent relation with the constraint i.e. if cost of swap would have been 1 and cost of remove be 5 then 5 swaps is equivalent to 1 remove that way things would have been different and may be more difficult 
// so here because of such value 1e12 , 1e12+1 , either we swap an element to get it right or remove it no other possibilith of moving an element from one and to other because it will definitely cost more than removing it 
// for example 10000 here moving one require 4 swaps so 4(1e12) and removing 1e12 + 1 , if cost of swap = 1 and cont of remove = 5 then swapping would have been better
void solve(vi& p)
{
    string arr;cin>>arr;
    ll n = arr.size();
    vi preOne(n,0),preZero(n,0);
    ll one = 0,zero = 0;
    REP(i,0,n)
    {
        if(arr[i]-'0')//1
    ...
    debug(preOne,preZero);
    ll ans = min(one,zero)*(1e12 + 1);
    REP(i,0,n)
    {
        // without swap
        ll one_on_left = preOne[i];
        ll zero_on_right = zero - preZero[i];
        ll curr = (1e12 + 1)*(one_on_left+zero_on_right);
        ans = min(ans , curr);
        if(i+1 < n)
        {
            // with swap
            if(arr[i] == '1' && arr[i+1] == '0')
            {
                ...
                zero_on_right = 0;
                if(i-1>=0)
                one_on_left = preOne[i-1];
                if(i+1<n)
                zero_on_right = zero - preZero[i+1];

                curr = (1e12+1)*(one_on_left+zero_on_right) + (1e12);
                ans = min(ans , curr);
            }
        }
    }
