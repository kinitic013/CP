...
void solve(ll t)
{
    ll curr_bit_count;cin>>curr_bit_count;
    ll n = 0;
    ll counter = 0;
    ll sub = 1;
    bool last_set = 0;
    ...
    while(curr_bit_count && i<30)
    {
        ...
        n+=sub;
        ll new_curr_bit;cin>>new_curr_bit;
        if(new_curr_bit+1 == curr_bit_count)// ith bit was set
        {
            sub *=2;
        }
        else
        {
            ll diff = new_curr_bit - curr_bit_count;
            next_set = i + diff + 1;   
        }
        debug(n,sub);
        curr_bit_count = new_curr_bit;
        counter++;
    }
    // cout<<counter<<endl;
    cout<<"! "<<n<<endl;
...
void toBinaryString(int num) {
    cout<<std::bitset<32>(num).to_string()<<endl; 
}
void testSolve(ll t)
{
    ll number = 939507712;
    ll curr_bit_count = __builtin_popcount(number);
    ll n = 0;
    ll counter = 0;
    ll sub = 1;
    bool last_set = 0;
    ...
    while(curr_bit_count )
    {
        ...
        number-=sub;
        n+=sub;
        ll new_curr_bit = __builtin_popcount(number);
        toBinaryString(number);
