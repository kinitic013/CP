...
void solve()
{
    ...
    REP(j,0,arr[trump-'A'].size())
    {
        extra.pb(arr[trump - 'A'][j]);
    }
    ll i =0;
    ll j = extra.size()-1;
    while(i<=j)
    {
        if(extra[i][1] != extra[j][1] && extra[i][1] != trump && extra[j][1] != trump)
        {
            cout<<"IMPOSSIBLE\n";
            return;
        }
        if(extra[i][1] == extra[j][1] && extra[i][0] > extra[j][0])
        {
            cout<<"IMPOSSIBLE\n";
            return;
        }
        res.pb({extra[i],extra[j]});
        i++;
        j--;
    }
    {
