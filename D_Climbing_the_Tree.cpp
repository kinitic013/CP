...
bool canAdopt(pii& h ,ll a, ll b,ll n)
{
    pii proposedHeight = {0,0};
    proposedHeight.ss = (n-1)*(a-b) + a;
    proposedHeight.ff = (n>1) ? ((n-2)*(a-b) +a + 1) : 1 ;
    proposedHeight.ff = max<ll>(proposedHeight.ff , 0);

    if(h.ff > proposedHeight.ss || proposedHeight.ff > h.ss)// non intersecting
    {
        return 0;
    }
    else
    {
        h.ff = max(h.ff,proposedHeight.ff);
        h.ss = min(h.ss,proposedHeight.ss);
        return 1;
    }
}
ll canDetermine(pii& h , ll a, ll b)
{
    ll dayMin , dayMax ;
    if(h.ff == LONG_LONG_MIN && h.ss == LONG_LONG_MAX)
    {
        return -1;
    }
    dayMin = (h.ff - a)/(a - b);
...
{
    int q;cin>>q;
    pii h = {LONG_LONG_MIN,LONG_LONG_MAX};
    REP(i,0,q)
    {
        ll state;cin>>state;
        if(state == 1)
        {
            ll a,b,n;cin>>a>>b>>n;
            cout<<canAdopt(h,a,b,n)<<" ";
        }
        else
        {
            ll a,b;cin>>a>>b;
            cout<<canDetermine(h,a,b)<<" ";
        }
    }
    return;
}
