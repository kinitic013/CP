...
vi  getZarr(string arr,string pattern)
{
    ll m = pattern.size();
    string str = pattern + "#" + arr; 
    ll n = str.length();
    vi  Z(n);
    ll L, R, k;
    L = R = 0;
...
void solve()
{
    ...
    ll n = arr.size();
    ll q;cin>>q;
    vector<string> pattern(q);
    vvi matrix(q);
    REP(i,0,q)
    {
        cin>>pattern[i];
        matrix[i] = getZarr(arr,pattern[i]);
    }
    vi dp(n,LONG_LONG_MAX);
    vpii res(n);
    REP(i,0,n)
    {
        REP(j,0,q)
        {
            if(matrix[j][i] == pattern[j].size())
            {
                ll last = 0;
                if(i-1>=0)
                last = dp[i-1];

                REP(k,i,min<ll>(n,i+pattern[j].size()))
                {
                    if(dp[k]>last+1)
                    {
                        dp[k] = last + 1;
    ...
    pii last = {LONG_LONG_MAX,LONG_LONG_MAX};
    cout<<dp[n-1]<<endl;
    vpii ans;
    ll i =n-1;
    while(i>=0)
