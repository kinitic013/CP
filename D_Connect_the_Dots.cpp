...
{
    int n,Q;cin>>n>>Q;
    vector<vector<pii>> queries(11);
    // d , {}
    REP(i,0,Q)
    {
        vi temp(3);cin>>temp[0]>>temp[1]>>temp[2];
        if(temp[2]!=0)
        queries[temp[1]].pb({temp[0],temp[2]});
    }
    // Observation 1 : we can preprocess all queries and then give the result
    // Observation 2 : d<= 10 small so we can iterate over all d seperately
    // Observation 3 : for any d , components are connected after each 'd' cycle ,for that i used queue kind of structure that will hold upcominng color to ith node and -1 if no query is placed before
    // Observation 4 : use DSU for coloring rather than manually coloring the nodes 
    // Time Complexity : O(Dmax*n)) 
    DSU arr(n);
    REP(d,1,11)// for every d
    {
        sort(all(queries[d]));
        queue<pii> q;
        ...
        while(i<n+1)
        {
            ...
            while(pointer<queries[d].size() && queries[d][pointer].ff == i) // matching new query added
            {
                if(top.ff == -1)
                top.ff = arr.findParent(queries[d][pointer].ff);
                else
                arr.sizeunion(top.ff,queries[d][pointer].ff);

                top.second = max(top.second,queries[d][pointer].second);
                pointer++;
            }

            {
                if(top.first == -1)// no color
                {
                    // still no color so continue 
                }
                else
                {
                    ll pv = arr.findParent(i);
                    arr.sizeunion(top.ff,pv);
                }
            }
            top.second--;
            if(top.second < 0)
