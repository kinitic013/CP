...
ll count_dfs(vvi& adj,vb& visited,vi& count,ll node )
{
    if(visited[node])
    return count[node];
    ...
    {
        if(!visited[x])
        curr += count_dfs(adj,visited,count,x);
        else
        curr += count[x]; 
    }
    curr++;
...
bool check_cycle(vvi& adj,vb& firstVisited,vb& secondVisited,vi& count,ll k,ll node)
{
    firstVisited[node] = 1;
    secondVisited[node] = 1;
    bool res = 1;
    for(auto x : adj[node])
    {
        if(firstVisited[x] && secondVisited[x])// cycle detected
        {
            ll curr = count[x];
            if(curr != k)
            res = false;
        }
        else if(!firstVisited[x])
        {
            bool check = check_cycle(adj,firstVisited,secondVisited,count,k,x);
            res = res&&check;
        }
    }
    secondVisited[node] = 0;
    return res;
}
...
void solve(vi& p)
{
    ...
    REP(i,1,n+1)
    {
        ...
        ll u = i;
        ll v = arr[i];
        // edge i -> arr[i] means ith index need arr[i]
        adj[u].pb(v);
    }
    if(k == 1)
    {
    ...
    vi count(n+1,0);
    vb firstVisited(n+1,false);
    vb secodnVisited(n+1,false);
    REP(i,1,n+1)
    {
        if(!firstVisited[i])
        {
            count_dfs(adj,firstVisited,count,i);
        }
    }
    debug(count);
    firstVisited = vb(n+1,false);
    bool res = 1;
    REP(i,1,n+1)
    {
        if(!firstVisited[i])
        res = res && check_cycle(adj,firstVisited,secodnVisited,count,k,i);
    }
    if(res)
    cout<<"YES";
    else
