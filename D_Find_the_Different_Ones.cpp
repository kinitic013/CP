...
void build(vector<ll>& arr, ll start , ll end , ll index)
{
    ...
    if(segTree[left].ff != segTree[left].ss)
    {
        segTree[index] = segTree[left];
    }
    else if(segTree[right].ff != segTree[right].ss)
    {
        segTree[index] = segTree[right];
    }
    else
    {
        if(arr[(segTree[left].ff)] == arr[(segTree[right].ff)])
        {
            segTree[index] = segTree[left];
        }
        else
        {
            segTree[index] = {segTree[left].ff,segTree[right].ff};
        }
    }
}
pii query(vi& arr,ll start,ll end , ll index , ll l , ll r)
{
    ll left = index*2 + 1;
    ll right = index*2 + 2;
    ll mid = (start + end )/2;
    ...
    {
        pii leftValue = query(arr,start,mid,left,l,r);
        pii rightValue = query(arr,mid+1,end,right,l,r);

        if((leftValue.ff != -1 && leftValue.ss != -1) && (rightValue.ff != -1 && rightValue.ss != -1))
        {
            if(leftValue.ff != leftValue.ss)
            {
                return leftValue;
            }
            if(rightValue.ff != rightValue.ss)
            {
                return rightValue;
            }
            if(arr[leftValue.ff] == arr[rightValue.ff])
            {
                return {leftValue.ff,leftValue.ff};
            }
            else
            {
                return { leftValue.ff,rightValue.ff};
            }
        }
        else
        {
            if(leftValue.ff != -1 && leftValue.ss != -1) 
            {
                return leftValue;
            }
            if(rightValue.ff != -1 && rightValue.ss != -1) 
            {
                return rightValue;
            }
            return inf;
...
void solve()
{
    ...
    REP(i,0,m)
    {
        ...
        l--;
        r--;
        pii result = query(arr,0,n-1,0,l,r);
        if(result == inf)
        {
        ...
        {
            if(result.ff == result.ss)
            {
                cout<<-1<<" "<<-1<<endl;
            }
            else
            {
                cout<<result.ff+1<<" "<<result.ss+1<<endl;
            }
        }
