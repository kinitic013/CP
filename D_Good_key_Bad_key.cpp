...
void solve()
{
    ...
    debug(dp);
    vvi matrix(32,vi(n,0));
    REP(i,0,32)
    {
        matrix[i][n-1] = dp[i][n-1];
    }
    debug(matrix);
    for(int i=31;i>=0;i--)
    {
        for(int j=n-1;j>=0;j--)
        {
            ...
            matrix[i][j] = dp[i][j] + prev; 
            debug(i,j,matrix[i][j],dp[i][j]);
        }
    }
    debug(matrix);
    ll ans = 0;
    REP(i,0,n)
    {
        ...
        if(i-1>=0)
        {
            before = pre[i-1] - cost[i-1];
        }
        ll curr = before + after;
        ans = max<ll>(ans,curr);
    }
    ans = max<ll>(ans,pre[n-1]-cost[n-1]);
    cout<<ans;
    return;
}
int main()
