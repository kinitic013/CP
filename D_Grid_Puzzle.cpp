...
void solve(ll t)
{
    ...
    vvi dp(n,vi(4,INT_MAX));// 0 , 2L , 4 , 2R
    // dp[i][j] = minmum operation to optain configuratoin with all 0 -> i-1 rows while and j configuration on ith row
    if(arr[0] == 0)
    {
        dp[0] = {0,INT_MAX,INT_MAX,INT_MAX};
    }
    else if(arr[0] == 2)
    { 
        dp[0] = {1,0,INT_MAX,INT_MAX};
    }
    else if(arr[0] == 4)
    {
        dp[0] = {1,1,0,1};
    ...
    {
        dp[0] = {1,INT_MAX,INT_MAX,INT_MAX};
    }
    REP(i,1,n)
    {
        if(arr[i] == 0)
        {
            dp[i][0] = min<ll>(dp[i-1][0],min<ll>(dp[i-1][1] + 1 ,min<ll>(dp[i-1][2] + 1,dp[i-1][3]+1)));
            dp[i][1] = INT_MAX;
            dp[i][2] = INT_MAX;
            dp[i][3] = INT_MAX;
        }
        ...
        {
            dp[i][0] = min<ll>(min<ll>(1+dp[i-1][0],2 + dp[i-1][3]) , min<ll>(1 + dp[i-1][1],2 + dp[i-1][2]));
            dp[i][1] = min<ll>(min<ll>(dp[i-1][0],1 + dp[i-1][3]),min<ll>(1 + dp[i-1][1],2 + dp[i-1][2]));
            dp[i][2] = INT_MAX;
            dp[i][3] = INT_MAX;
        }
        else if(arr[i] == 4)
        {
            dp[i][0] = min<ll>(1 + dp[i-1][0] , 2 + min<ll>(dp[i-1][1],min<ll>(dp[i-1][2],dp[i-1][3])));
            dp[i][1] = min<ll>(min<ll>(1 + dp[i-1][0],2 + dp[i-1][1]),min<ll>(2 + dp[i-1][2],1 + dp[i-1][3])) ;
            dp[i][2] = min<ll>(dp[i-1][0] , 1 + min<ll>(dp[i-1][1],min<ll>(dp[i-1][2],dp[i-1][3])));
            dp[i][3] = min<ll>(1 + dp[i-1][1],2 + min<ll>(dp[i-1][0],min<ll>(dp[i-1][2],dp[i-1][3])));
        }
        else
        {
            dp[i][0] = min<ll>(1 + dp[i-1][0] , 2 + min<ll>(dp[i-1][1],dp[i-1][2]));
            dp[i][1] = INT_MAX;
            dp[i][2] = INT_MAX;
            dp[i][3] = INT_MAX;
        }
    }
    cout<<min<ll>(dp[n-1][0],1 + min<ll>(dp[n-1][1],min<ll>(dp[n-1][3],dp[n-1][2])));
}
int main()
{
    fast();
