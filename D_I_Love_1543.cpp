...
bool is(vi& a,ll start)
{
    ll n = a.size();
    if(a[start] == 1 && a[(start+1)%n] == 5 && a[(start+2)%n] == 4 && a[(start+3)%n] == 3)
    return 1;
    else
    return 0;
}
ll count(vector<string>& arr,ll start_row,ll start_column)
{
    ll n = arr.size();
    ll m = arr[0].size();
    ll end_row = n - 1 - start_row;
    ll end_column = m - 1 - start_column;
    vi last;
    ll i = start_column;
    ll res = 0;
