...
void solve()
{
    ...
    REP(i,0,n-1)
    {
        principal += abs(arr[i] - arr[i+1]);
    }
    pii l = {1,mini-1};
    pii h = {maxi +1,x};
    debug(l,h,mini,maxi,arr);
    ll lower = LONG_LONG_MAX;
    ll higher = LONG_LONG_MAX;
    if(l.ff<=l.ss)
    {
        REP(i,0,n)
        {
            if(i>0 && i<n-1)// in the mid
            {
                lower = min<ll>(lower,2*abs(arr[i] - 1));
            }
            else
            {
                lower = min<ll>(lower , abs(arr[i] - 1) );
            }
            debug(lower,arr[i]);
        }
    }
    if(h.ff<=h.ss)
    {
        REP(i,0,n)
        {
            if(i>0 && i<n-1)// in the mid
            {
                higher = min<ll>(higher,2*abs(arr[i] - x));
            }
            else
            {
                higher = min<ll>(higher , abs(arr[i] - x) );
            }
        }
    } 
    if(lower == LONG_LONG_MAX) lower = 0;
    if(higher == LONG_LONG_MAX ) higher = 0;


    cout<<principal + lower + higher;
    return;
}
int main()
{
