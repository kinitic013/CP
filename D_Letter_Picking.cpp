...
void solve(vi& p)
{
    ...
    vvi dp(n,vi(n,0));// o -> alice, 2->bob , 1->draw
    ...
    while(diff < n)
    {
        ...
        while(i<n && j<n)
        {
            ...
            // if ith element is choosen by alice
            {
                // if bob choose i+1 th elment
                ans1 = dp[i+2][j];
                if(ans1 == 1)
                {
                    if(arr[i] != arr[i+1])
            ...
            // if jth element is chooen by alice
            {
                // j-1th element is by bob
                ans3 = dp[i][j-2];
                if(ans3 == 1)
                {
                    if(arr[j] != arr[j-1])
                ...
                // if ith element is choosen by bob
                ans4 = dp[i+1][j-1];
                if(ans4 == 0)
                {
                    if(arr[i] != arr[j])
            ...
            dp[i][j] = min(max(ans1,ans2),max(ans3,ans4));
            i++;
            j++;
        }
        diff+=2;
