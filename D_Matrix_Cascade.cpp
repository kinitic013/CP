...
void solve(vi& p)
{
    ...
    vvi dpplus(n,vi(n+1,0));
    vvi dpminus(n,vi(n+1,0));
    ll count = 0;
    REP(i,0,n)
    {
        ll curr = 0;
        REP(j,0,n)
        {
            debug(dpplus,dpminus);
            curr += dpplus[i][j]+dpminus[i][j];
            ll left = max<ll>(0,j-1);
            ll right = min<ll>(n,j+1);
            if(i+1<n)
            {
                dpplus[i+1][left] += dpplus[i][j];
                dpminus[i+1][right] += dpminus[i][j]; 
            }
            ll state = arr[i][j] - '0';
            if(curr&1)
            {
            ...
            if(state == 1)// need to use an operation 
            {
                count++;
                if(i+1<n)
                {
                    dpplus[i+1][left]++;
                    dpminus[i+1][min<ll>(right+1,n)]--;
                }
            }
        }
    }
