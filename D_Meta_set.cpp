    ...
    REP(i,0,n)
    {
        REP(j,i+1,n)
        {
            ...
            REP(l,0,k)
            {
                ...
                need[l] = 3 - arr[i][l] - arr[j][l];
            }
            // debug(arr[i],arr[j],need);
            ll idx;
            bool check = t.search(need,idx);
            if(check)
            {
                count[idx]++;
                debug(i,j,idx);
