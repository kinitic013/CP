...
void solve(vi& p)
{
    ...
    vi arr(n);
    REP(i,0,n) cin>>arr[i];
    vi rightSmaller = nextSmallerRight(arr);
    vi leftSmaller = nextSmallerLeft(arr);
    vi range(n,0);
    vpii list;
    
    REP(i,0,n)
    {
        range[i] = rightSmaller[i] - leftSmaller[i] - 1;
        list.pb({range[i],arr[i]});
    }
    sort(all(list),cmps);
    debug(list,range);
    ll curr = 0;
    ...
    for(int i=0;i<n && j>0;i++)
    {
        if(list[i].ff + 1 < j)
        {
            ...
            {
                j--;
                res[j] = max(res[i],curr);
            }
        }
        if(list[i].ff == j || list[i].ff == j-1)
        {
            curr = max<ll>(curr , list[i].ss);
            j = list[i].ff;
            res[j] = curr; 
        }
    }
