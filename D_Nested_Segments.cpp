...
void solve(ll t)
{
    ...
    REP(i,0,2*n)
    {
        auto it = compress.find(arr[i]);
        if(it == compress.end())
        {
            compress[arr[i]] = val;
            compress[m[arr[i]]] = val;
            arr[i] = val;
            val++;
        }
        else
    ...
    REP(i,0,2*n)
    {
        ...
        {
            res[arr[i]] = query(0,2*n-1,0,first[arr[i]],i);
            update(visited,0,2*n-1,0,first[arr[i]],1);
        }
    }
    for(int i=0;i<n;i++)
    {
        ll mapping = compress[edges[i].ff];
        cout<<res[mapping]<<"\n";
    }
