...
void dfs(vvi& adj,ll node,ll parent,vi& res,vi& s)
{
    if(node != 1)
    {
        auto it = s.begin();
        while(1)
        {
            ll randomIndex = std::rand() % s.size();
            ll ele = s[randomIndex];
            if(lp[abs(ele - res[parent])] != abs(ele - res[parent]))
            {
                res[node] = ele;
                swap(s[randomIndex],s[s.size()-1]);
                s.pop_back();
                break;
            }
            it++;
