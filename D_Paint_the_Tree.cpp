...
bool DFS_PATH(vvi&adj,ll a,ll b,vb& visited,ll node,vi& path)
{
    if(visited[node])
    return false;
    visited[node] = 1;
    ...
    for(auto x : adj[node])
    {
        if(!visited[x])
        {
            bool check = DFS_PATH(adj,a,b,visited,x,path);
            if(check)
            {
                path.pb(node);
                return 1;
...
ll DFS_DEPTH(vvi& adj,vb& visited, ll node )
{
    if(visited[node])
    return 0;
    ...
    for(auto x : adj[node])
    {
        if(!visited[x])
        {
            ll curr = DFS_DEPTH(adj,visited,x);
...
void solve(vi& p)
{
    ...
    DFS_PATH(adj,a,b,visited,a,path);
    reverse(all(path));
    ll node = -1;
    ll ans = 0;
    if(path.size()&1)
    ...
    {
        node = path[(path.size()-1)/2];
        ans+=(path.size())/2;
    }
    visited = vb(n+1,false);
    ll max_depht = DFS_DEPTH(adj,visited,node)-1;
    ans += 2*(n-1) - max_depht;
    cout<<ans;
}
int main()
