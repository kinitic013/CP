...
vi query(vector<vvi>& sparse,ll n,ll m,ll l, ll r,ll isIdempotent=true)
{
    ll size = r - l + 1;
    ll range = log2(size);
    ll sum = 0;
    ...
    if(isIdempotent)
    {
        REP(i,0,m)
        {
            temp.push_back(max(sparse[i][l][range],sparse[i][r - (1<<range) +1][range]));
        }
        return temp;
    }
    else
...
void solve(ll t)
{
    ...
    vector<vvi> sparse(m,vvi(n,vi(20,LONG_LONG_MIN)));
    REP(i,0,m)
    {
        REP(l,0,n)
        {
            sparse[i][l][0] = arr[l][i];
        }
        REP(j,1,20)
        {
            REP(l,0,n)
            {
                if(l + (1<<(j-1))<n)
                {
                    ll curr_val = max(sparse[i][l][j-1],sparse[i][l+(1<<(j-1))][j-1]);
                    sparse[i][l][j] = curr_val;
                }
            }
        }
    }
    ...
    while(r-l>1)
    {
        ...
        REP(i,0,n)
        {
            if(i + mid - 1 < n)
            {
                vi temp = query(sparse,n,m,i,i+mid-1,1);
                ll sum = 0;
