...
void solve(vi& p)
{
    ...
    if(n%2==0)
    {
        // K-degree graph is Eulerian Graph so there can be bridge that divides graph into SCCs 
        cout<<"NO";
        return;
    }
    cout<<"YES"<<endl;
    cout<<(4*n)-2<<" "<<(2*(((n-1)*(n-1))+(n-1) + ((n-1)/2))+1)<<endl;
    cout<<1<<" "<<2*n<<endl;
    printEdges(n,1);
    printEdges(n,2*n);
}
