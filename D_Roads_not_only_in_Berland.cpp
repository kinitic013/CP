...
void solve(ll t)
{
    ...
    {
        ll u,v;cin>>u>>v;
        if(arr.findParent(u) == arr.findParent(v))// already in a connected component
        {
            edges_to_remove.pb({u,v});
        }
        else
        {
            arr.sizeunion(u,v);
    ...
    REP(i,1,n+1)
    {
        components.insert(arr.findParent(i));
    }
    auto it = components.begin();
    it++;
    ll val = *components.begin();
    cout<<edges_to_remove.size()<<endl;
    REP(i,0,edges_to_remove.size())
    {
        cout<<edges_to_remove[i].ff<<" "<<edges_to_remove[i].ss<<" "<<val<<" "<<*it<<endl;
        it++;
    }
