...
vector<int> segTree;
void build(vi& arr, ll start , ll end , ll index)
{
    if(start == end)
    {
...
void solve(vi& p)
{
    ll n,m;cin>>n>>m;
    vector<ll> arr(m+1);
    vi diff(m+1,INT_MAX-2);
    REP(i,1,m+1)
    {
        cin>>arr[i];
        diff[i] = n - arr[i];
    }
    segTree.resize(4*m,INT_MAX);
    build(diff,0,m,0);
    ll q;cin>>q;
    REP(x,0,q)
    {
        ll a,b,c,d,k;cin>>a>>b>>c>>d>>k;
        // check if a,b and c,d are in unblocked zone of matrix
        debug(arr[b],a);
        debug(arr[d],c);
        if((arr[b]>=a) || (arr[d]>=c))
        {
        ...
        // difference between a,c and b,d must be divisible by k
        if(abs(a-c)%k!=0 || abs(b-d)%k != 0)
        {
            cout<<"NO\n";
            continue;
        }
        // now there should be a path between bth column to dth column
        ll mini = query(0,m,0,min(b,d),max(b,d));
        ll multi = (n - a)/k;
        ll point = a + (multi*k);
        if(n - point <= mini-1)
        cout<<"YES\n";
