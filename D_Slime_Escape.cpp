...
void solve(ll t)
{
    ...
    ll l = k-1;
    ll r = k+1;
    ll left_sum = arr[l] , right_sum = arr[r];
    if(k == 0 || k == n-1)
    {
        cout<<"YES";
        return;
    ...
    while(l>=0 && r < n)
    {
        // try to move on the left side
        {
            if(left_sum + right_max >= 0)
            {
                l--;
                left_max = max(left_max , left_sum + arr[k]);
                if(l>=0)
                left_sum += arr[l];
                continue;
            }
        }
        // try to move on the right side
        {
            if(right_sum + left_max >= 0)
            {
                r++;
                right_max = max(right_max , right_sum + arr[k]);
                if(r<n)
                right_sum += arr[r];
                continue;
            }
