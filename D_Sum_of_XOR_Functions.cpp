...
void solve(vi& p)
{
    ...
    REP(i,0,32)
    {
        ...
        vpii dps(n,{0,0});// dpStart i.e. |_________>
        if(str[0] == 1)
        dps[0] = {1,0};
        else
        dps[0] = {0,1};
        ...
        REP(j,1,n)
        {
            if(str[j] == 1)
            {
                dps[j] = {dps[j-1].ss+1,dps[j-1].ff};
            }
            else
            {
                dps[j] = {dps[j-1].ff,dps[j-1].ss + 1};
            }
        }
        REP(j,0,n)
        {
            curr = (curr + ((dps[j].ff)*(j+2)%mod))%mod;
        }

        vpii dpe(n,{0,0});//dpEnd <________|
        if(str[n-1] == 1)
        dpe[n-1] = {1,0};
        else
        dpe[n-1] = {0,1};
        ...
        for(int j = n-2;j>=0;j--)
        {
            if(str[j] == 1)
            {
                dpe[j] = {dpe[j+1].ss+1,dpe[j+1].ff};
            }
            else
            {
                dpe[j] = {dpe[j+1].ff,dpe[j+1].ss + 1};
            }
        }
        REP(j,0,n)
        {
            curr = ((curr - (dpe[j].ff)*(j+1) + mod))%mod;
        }
