...
bool checker(vvi& arr , ll l) // used dp 
{
    ll n = arr.size();
    ll m = arr[0].size();
    vvi row(n,vi(m,0));
    ...
    REP(j,0,m)
    {
        REP(i,0,n)
        {
            ...
            if(i==0 || j==0)
            {
                square[i][j] = min(square[i][j],column[i][j]);
            }
        }
    }
    debug(row,column);
    // square[i][j]  = maximum possible square with all element at least l with bottom right corner at i,j
    REP(i,1,n)
    {
        REP(j,1,m)
        {
            ll prev = square[i-1][j-1] + 1;
            ll r = row[i][j];
            ll c = column[i][j];

            square[i][j] = min<ll>(prev,min<ll>(r,c));
        }
    }
    debug(square);
    REP(i,0,n)
