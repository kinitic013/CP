...
void solve(vi& p)
{
    ...
    n = count.size();
    vvi dp(n,vi(n+10,INT_MAX));
    // dp[i][j] = optimally minimum alice would be able to taste for first i elements in the array with j good moves remaining for bob 
    // good move = free moves that bob could use to save some cake from alice before her/his move
    dp[0][0] = 1;
    dp[0][1] = 1;
    REP(i,0,n-1)
    {
        REP(j,0,n+1)
        {
            // saveing bob's current move 
            dp[i+1][j+1] = min(dp[i+1][j+1],dp[i][j]+1);
            // spending all j moves to save arr[i+1] 
            if(j!=0)
            {
                if(count[i+1]<=j)// can be saved
                {
                    dp[i+1][j-count[i+1]] = min(dp[i+1][j-count[i+1]],dp[i][j]);
                }
                else// can't be saved
                {
