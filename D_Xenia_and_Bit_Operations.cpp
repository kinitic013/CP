...
void build(vector<int>& arr, int start , int end , int index,int level)
{
    if(start == end)
    {
    ...
    int left = index*2+1;
    int right = index*2 + 2;
    build(arr,start,mid,left,level-1);
    build(arr,mid+1,end,right,level-1);
    int new_val = 0;
    if(level&1) // or operation
        new_val = segTree[left]|segTree[right];
    else 
        new_val = segTree[left]^segTree[right];
    
    segTree[index] = new_val;
}
void update(vector<int>& arr , int start , int end , int index , int pos , int value , int level)
{
    if(start == end)
    {
        arr[pos] = value;
    ...
    int right = index*2 + 2;
    if(pos<=mid)
        update(arr,start,mid,left,pos,value,level-1);
    else
        update(arr,mid+1,end,right,pos,value,level-1);
...
int query(int start,int end , int index , int l , int r, int level)
{
    int left = index*2 + 1;
    int right = index*2 + 2;
    int mid = (start + end )/2;
    ...
    {
        int leftValue = query(start,mid,left,l,r,level-1);
        int rightValue = query(mid+1,end,right,l,r,level-1);
...
void solve(ll t)
{
    ...
    segTree.resize(4*(1<<n),0);
    build(arr,0,(1<<n)-1,0,n);

    REP(i,0,m)
    {
        ll pos,val;cin>>pos>>val;
        update(arr,0,(1<<n)-1,0,pos-1,val,n);
        cout<<segTree[0]<<"\n";
    }
    return;
}
