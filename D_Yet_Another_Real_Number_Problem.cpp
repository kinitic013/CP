...
void solve(ll t)
{
    ...
    REP(i,1,n)
    {
        ...
        // Value from last
        {
            ll expo_in_range = pow[i];
            if (last[i - 1] >= 0)
                expo_in_range -= pow[last[i - 1]];

            ll sum_in_range = pre[i];
            if (last[i - 1] >= 0)
                sum_in_range -= pre[last[i - 1]];

            ll power_last_val = power(2, expo_in_range, MOD);
            ll add1 = (power_last_val * base[i]) % MOD;

            val1 = dp[last[i - 1]] + add1 + sum_in_range - base[i];
        }

        // Value from current
        {
            ll res = power(2, pow[i], MOD);
            ll add1 = (res * base[i])%MOD;
            val2 = add1 + pre[i] - base[i];
        }

        if (val2 != val1) {
            // If counts are different, choose based on count comparison
            if (count1 > count2 || (count1 == count2 && val1 >= val2)) {
                dp[i] = val1;
                last[i] = last[i - 1];
            } else {
                dp[i] = val2;
            ...
            // If val1 == val2, choose val2 and update last
            dp[i] = val2;
            last[i] = i;
        }
    }
