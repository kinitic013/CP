...
set<ll> dfs(vvi& adj,vi& color,vi& res,ll node,ll parent)
{
    set<ll> curr;
    for(auto x : adj[node])
    {
        if(x != parent)
        {
            set<ll> child_set = dfs(adj,color,res,x,node);
            // if(child_set.size() > curr.size())
            // {
            //     swap(child_set,curr);
            // }

            auto it = child_set.begin();
