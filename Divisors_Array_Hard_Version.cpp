...
map<ll, ll> count_of_divisor(ll n) {
    map<ll, ll> ans;
    for (int i = 2; i <= n; ++i) {
        if (lp[i] == i) {
            ll c = 0;
...
ll divisor_count(map<ll,ll>& curr,ll n,ll val)
{
    ll ele = n;
    map<ll,ll> fre;
    while(ele > 1)
    ...
    while(it != fre.end())
    {
        if(curr[it->ff] != 0)
        {
            ans = (ans * modInverse(curr[it->ff]+1,MOD))%MOD;
        }
        ans = (ans * (curr[it->ff] + it->ss + 1))%MOD;
        it++;
    }
    return ans;
}
void solve(ll t)
{
    ...
    map<ll,ll> fre = count_of_divisor(m);
    debug(fre);
    auto it = fre.begin();
    ll val = 1;
    while(it != fre.end())
    ...
    REP(i,0,n)
    {
        ll ans = divisor_count(fre,arr[i],val);
        cout<<ans<<" ";
    }
    return;
}
