...
void solve(ll t)
{
    ...
    REP(i,0,n)
    {
        ...
        {
            debug(minHeap,que);
            ll minTop = *minHeap.begin();
            ll top = que.front();
            dp[i][j] = arr[i][j] + minTop + 1;
            if(minHeap.size()>=(d+1))
            {
                auto it = minHeap.find(top);
                minHeap.erase(it);
                que.pop();
            }

            que.push(dp[i][j]);
            minHeap.insert(dp[i][j]);
            debug(minHeap,que,dp[i][j]);
        }
    }
    debug(dp);
    vi res;
