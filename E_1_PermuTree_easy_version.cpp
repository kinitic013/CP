...
void dfs(vvi&adj,vi& dp,vi& node_sum,ll node)
{
    ll curr = 0;
    ll total_nodes = 0;
    ll curr_nodes = 0;
    vi arr;
    for(auto x : adj[node]){
        dfs(adj,dp,node_sum,x);
        dp[node] += dp[x];
        total_nodes += node_sum[x];
        arr.pb(node_sum[x]);
    }
    sort(all(arr));
    curr = max_product(arr);
    dp[node] += curr;
    node_sum[node] = 1 + total_nodes;
}
void solve(ll t)
{
    ll n;cin>>n;
