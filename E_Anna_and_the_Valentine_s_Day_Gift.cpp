...
void solve()
{
    ...
    struct Compare {
        bool operator()(const pair<long long, long long>& a, const pair<long long, long long>& b) {
            // Compare based on the first element of the pair (long long)
            return a.first < b.first;
        }
    };

    // Declare a max heap priority queue using the custom comparator
    priority_queue<pair<long long, long long>, vector<pair<long long, long long>>, Compare> q;

    REP(i,0,n)
    {
        string str;cin>>str;
        ll count = countzero(str);
        arr.pb({count,str.size()});
        q.push({count,str.size()});
    }
    ll state = 0;
    while(q.size() != 0)
    {
        ...
        else
        {
            ...
            pii top2 = q.top();
            q.pop();
            q.push({min(top1.ff,top2.ff) , top1.ss + top2.ss});
            state = 0;
        }
    }
    pii top = q.top();
