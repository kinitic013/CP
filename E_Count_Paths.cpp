...
pair<ll,map<ll,ll>> DFS(vector<vector<ll>>& adj,vi& color,vi& dp,vector<bool>& visited , ll node)
{
    if(visited[node])
    return {};
    visited[node] = 1;

    pair<ll,map<ll,ll>> res;
    ll count = 0;
    for(auto x : adj[node])
    {
        if(!visited[x])
        {
            pair<ll,map<ll,ll>> child = DFS(adj,color,dp,visited,x);
            debug(node,res,child);
            // merging
            count += child.ff ; // addding child answer
            
            auto add = child.ss.find(color[node]);
            if(add!=child.ss.end())
            count += add->ss; // adding with current node color is same as those present in subtree
            

            if(child.ss.size()>res.ss.size())
            {
                swap(child.ss,res.ss);
            }
            auto it = child.ss.begin();
            while(it != child.ss.end())
            {
                if(it->ff == color[node])
                {
...
void solve()
{
    ...
    count = (DFS(adj,color,dp,visited,1)).ff;
    cout<<count;
}
int main()
{
