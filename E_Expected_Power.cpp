...
// Returns x^(y) mod p
int power(int x, int y, int p=(int)(1e9+7))
{
    int res = 1;  
    x = x % p; 
    if (x == 0) return 0;
...
void solve(int t)
{
    ...
    REP(i,0,n) cin>>p[i];
    int denominator = modInverse(1e4, MOD);
    vi dp(LIMIT,0);
    // dp[i][j] = probability sum of all possible subsets with their xor as j from 0 till ith element in the array
    dp[arr[0]] = (1LL * p[0] * denominator) % MOD;
    dp[0] = (1LL * (10000 - p[0]) * denominator) % MOD;
    REP(i,0,n-1)
    {
        vi dp_next(LIMIT,0);
        REP(j,0,LIMIT)
        {
            // adding i+1th element to set
            if((arr[i+1]^j)<LIMIT)
            {
                dp_next[(arr[i+1]^j)] = (dp_next[(arr[i+1]^j)] + (1LL * dp[j] * ((1LL * p[i+1] * denominator) % MOD)) % MOD) % MOD;
            }
            // not adding i+1th element to set
            {
                dp_next[j] = (dp_next[j] + (1LL * ((1LL * (10000-p[i+1]) * denominator) % MOD) * dp[j]) % MOD) % MOD;
            }
        }
        dp = dp_next;
    }   
