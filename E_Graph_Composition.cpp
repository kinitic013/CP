...
void dfs(vvi& adj,vi& color , ll node,vb& visited , ll curr)
{
    color[node] = curr;
    visited[node] = 1;
    for(auto x : adj[node])
...
void solve(ll t)
{
    ...
    REP(i,0,m1)
    {
        ...
        {
            d.sizeunion(edges[i].ff,edges[i].ss);
        }
        else
        {
            res++;
    ...
    REP(i,1,n+1)
    {
        ll pu = d.findParent(i);
        if(pu == parent[color[i]])
        {
            continue;
        }
        else
        {
            d.sizeunion(pu,parent[color[i]]);
            res++;
        }
    }
    cout<<res;
