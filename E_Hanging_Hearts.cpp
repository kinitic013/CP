...
void dfs(vvi& adj,ll node,ll parent , vi& height,vi& leaf)
{
    for(auto x : adj[node])
    {
        if(x != parent)
        {
            dfs(adj,x,node,height,leaf);
            leaf[node] += max(leaf[x],height[x]);
            height[node] = max(height[x],height[node]);
        }
    }
    if(height[node] == 0)
    leaf[node] = 1;
...
void solve(ll t)
{
    ...
    vi height(n+1,0);
    vi leaf(n+1,0);
    dfs(adj,1,0,height,leaf);
    cout<<max(height[1],leaf[1]);
}
int main()
{
    fast();
