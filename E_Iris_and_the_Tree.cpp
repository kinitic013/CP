...
void dfs(vvi& adj,vvi& edges_to_path,vi& uk,vi& parent_arr,ll node,ll parent,ll& target)
{
    if(node == target)
    {
        edges_to_path[node].pb(target-1);
        uk[target-1]++;
        target++;
    }
    for(auto x : adj[node])
    {
        dfs(adj,edges_to_path,uk,parent_arr,x,node,target);
        edges_to_path[x].pb(target-1);
        uk[target-1]++;
    }
}
void solve(ll t)
{
    ll n,w;cin>>n>>w;
    vvi adj(n+1);
    vi parent_arry(n+1,0);
    REP(i,2,n+1)
    {
        ll parent;
        cin>>parent;
        adj[parent].pb(i);
        parent_arry[i] = parent;
    }
    debug(adj);
    vvi edges_to_path(n+1);
    vi uk(n+1,0);
    vi k (n+1,0);
    ll target = 2;
    dfs(adj,edges_to_path,k,parent_arry,1,0,target);
    debug(edges_to_path,uk);
    ll ans = 2*w;
    ll know = w;
    vpii edges_with_weight;
    REP(q,0,n-1)
    {
        ll x,tx;cin>>x>>tx;
        edges_with_weight.pb({x,tx});
    }
    vi res;
    res.pb(2*w);
    ll count_uk = 0;
    for(int i=n-2;i>0;i--)
    {
        ll weight = edges_with_weight[i].second;
        ll u = edges_with_weight[i].first;
        know -= weight;
        
        for(auto v : edges_to_path[u])
        {
            k[v]--;
            if(uk[v] == 0)
            {
