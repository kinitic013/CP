...
ll dfs_counter(vvi& adj,vb& loop,ll node,ll parent)
{
    ll count = 1;
    for(auto x : adj[node])
    {
        if(x != parent && loop[x]==0)
        {
            count += dfs_counter(adj,loop,x,node);
        }
    }
    return count;
}
void solve(ll t)
{
    ...
    ll start = -1;
    dfs_loop(adj,visited,loop,1,0,start);
    vi count(n+1,0);
    REP(i,1,n+1)
    {
        if(loop[i])
        {
            count[i] = dfs_counter(adj,loop,i,-1);
        }
    }
    debug(count);
    ll ans = 0;
