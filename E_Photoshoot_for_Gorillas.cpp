...
void create_prefix_2d(vector<vector<ll>>& arr) {
    int n = arr.size();
    int m = arr[0].size();
...
ll calculate_total_k_subsquare_sum(vector<vector<ll>>& prefix, int k) {
    int n = prefix.size() - 1;  // Original array size
    ...
    for (int i = k; i <= n; i++) {
        for (int j = k; j <= m; j++) {
            total_sum += prefix[i][j] - prefix[i-k][j] - prefix[i][j-k] + prefix[i-k][j-k];
        }
    }
...
void solve(ll t)
{
    ...
    REP(i,0,n)
    {
        REP(j,0,m)
        {
            ll h = min(i, n-k) - max<ll>(0, i-k+1) + 1;
            ll w = min(j, m-k) - max<ll>(0, j-k+1) + 1;
            arr[i][j] = h*w;
            pq.push({h*w,{i,j}});
        }
    }
    ...
    {
        pii pos = pq.top().ss;
        dp[pos.ff][pos.ss] = g[i];
        pq.pop();
    }
    create_prefix_2d(dp);
    debug(dp);
    cout<<calculate_total_k_subsquare_sum(dp,k);
    return;
}
int main()
{
