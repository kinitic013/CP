...
long long queryRange(long long start, long long end, long long index, long long l, long long r) {
    ...
    long long rightValue = queryRange(mid + 1LL, end, right, l, r);
...
void solve(ll t)
{
    ...
    val++;
    segTree.resize(4*val,0);
    lazySegTree.resize(4*val,0);
    vi range(val,0);
    for(auto x : arr)
    {
        rangeUpdate(range,0LL,val-1LL,0LL, m[x.first],m[x.second],1LL);
    }
    REP(i,0,n)
    {
        ll res = queryRange(0LL,val-1LL,0LL,m[arr[i].ff],m[arr[i].ss]);
        if(res > 1LL)
        {
            cout<<i+1LL;
            return;
