...
vvi segTree;
vvi lazySegTree;
void build(vector<long long>& arr, long long start , long long end , long long index,long long bit)
{
    if(start == end)
    {
        segTree[bit][index] = isKthBitSet(arr[start],bit);
        return; 
    }
    long long mid = (start + end)/2;
    long long left = index*2+1;
    long long right = index*2 + 2;
    build(arr,start,mid,left,bit);
    build(arr,mid+1,end,right,bit);
    segTree[bit][index] = (segTree[bit][left] + segTree[bit][right]);
}
void rangeUpdate(vector<long long>& arr, long long start, long long end, long long index, long long l, long long r , long long bit) {
    ...
    if (lazySegTree[bit][index] != 0) {
        segTree[bit][index] =(end - start + 1  - segTree[bit][index]);
        if (start != end) { // Propagate lazy value to children
            lazySegTree[bit][left] = lazySegTree[bit][left] ^ 1;
            lazySegTree[bit][right] = lazySegTree[bit][right] ^ 1;
        }
        lazySegTree[bit][index] = 0;
    }
    ...
    // Partial overlap
    rangeUpdate(arr, start, mid, left, l, r,bit);
    rangeUpdate(arr, mid + 1, end, right, l, r, bit);
    segTree[bit][index] = (segTree[bit][left] + segTree[bit][right]); // Update the current node
}

long long queryRange(long long start, long long end, long long index, long long l, long long r,long long bit) {
    long long segTreeSize = segTree.size();
    long long left = index * 2 + 1;
    long long right = index * 2 + 2;
    long long mid = (start + end) / 2;
    ...
    // Total overlap
    if (start >= l && end <= r) {
        return segTree[bit][index];
    }

    // Partial overlap
    long long leftValue = queryRange(start, mid, left, l, r,bit);
    long long rightValue = queryRange(mid + 1, end, right, l, r,bit);
...
void solve(ll t)
{
    ...
    REP(i,0,n) cin>>arr[i];
    segTree.resize(20);
    lazySegTree.resize(20);
    REP(i,0,20)
    {
        segTree[i].resize(4*n,0);
        lazySegTree[i].resize(4*n,0);
        build(arr,0,n-1,0,i);
    }
    // {
    //         ll sum = 0;
    //         REP(bit,0,20)
    //         {
    //             ll curr = queryRange(0,n-1,0,0,n-1,bit);
    //             sum += curr * (1<<bit);
    //         }
    //         cout<<sum<<endl;
    //         return;
    ...
    REP(i,0,m)
    {
        ...
        if(t == 2)
        {
            ...
            {
                if(v & (1<<bit))
                rangeUpdate(arr,0,n-1,0,l,r,bit);
            }
        }
        else
        {
            ...
            REP(bit,0,20)
            {
                ll curr = queryRange(0,n-1,0,l,r,bit);
                sum += curr * (1<<bit);
            }
            cout<<sum<<endl;
        }
