...
void solve(ll t)
{
    ...
    REP(i,0,(1<<n))
    {
        for(ll j = 0;j<n;j++)//bit
        {
            if((i&(1<<j)) == 0) // kth bit is unset
            {
                ll new_mask = i | (1<<j);// set kth bit
                if(dp[i].ss + arr[j] <= w)// placing without creating new elevator
                {
                    dp[new_mask] = cmps(dp[new_mask],{dp[i].ff,dp[i].ss+arr[j]});
                }
                else
                {
                    dp[new_mask] = cmps(dp[new_mask],{dp[i].ff+1,arr[j]});
                }
            }
        }
    }
