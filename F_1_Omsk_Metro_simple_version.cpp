...
ll dfs(vvi& adj, ll node, ll parent, vpii& minmax, vi& weight , ll mini , ll maxi)
{
    mini += weight[node];
    maxi += weight[node];
    if(maxi < 0)
    ...
    minmax[node].ff = max(maxi , minmax[parent].ff);
    minmax[node].ss = min(mini , minmax[parent].ss); 
    for(auto x : adj[node])
    {
        if(x != parent)
        {
            dfs(adj, x, node, minmax, weight,mini,maxi);
        }
    }
    return 0;
}
...
void solve(ll t)
{
    ...
    vpii minmax(n+10, {0, 0});
    ll mini = 0,maxi = 0;
    dfs(adj, 1, 0, minmax, weight , mini, maxi);
    REP(i, 0, query.size())
    {
        ll aim = query[i][2];
        ll v = query[i][1];
        if(minmax[v].ff >= aim && minmax[v].ss <= aim)
        {
            cout << "YES\n";
        } 
        else
