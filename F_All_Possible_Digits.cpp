...
void solve(ll t)
{
    ...
    debug(arr,brr,mini,maxi);
    ll ans = p-1;
    if(mini > arr[n-1] )// carry not needed
    {
        ans = max(0LL,maxi - arr[n-1]);   
    }
    else
    {
        ans = p - arr[n-1];
        ...
        ll last_element_before = arr[n-1] - 1;
        for(int i=n-1;i>=0;i--)
        {
            if(brr[i]>last_element_before)
            continue;
            else if(brr[i] == last_element_before)
            last_element_before--;
            else
            break;
        }
        if(last_element_before == new_element)
        last_element_before--;
        for(int i=n-1;i>=0;i--)
        {
            if(brr[i]>last_element_before)
        ...
        ans += max(0LL,last_element_before);
    }
    cout<<ans;
}
int main()
