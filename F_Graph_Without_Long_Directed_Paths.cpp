...
void dfs(vvi& adj,string& arr,vb& visited,ll node,ll parent)
{
    if(visited[node])
    return;
    ...
    for(auto x : adj[node])
    {
        if(x != parent && visited[x] == 0)
            dfs(adj,arr,visited,x,node);
    }
}
bool check(vvi& adj,string& arr,vb& visited,ll node,ll parent)
{
    if(visited[node])
    return 1;
    ...
    for(auto x : adj[node])
    {
        ...
        if(x != parent && visited[x]==0)
        result &= check(adj,arr,visited,x,node);
    }
    return result;
}
void solve(ll t)
{
    ...
    REP(i,0,n+1)
    res.pb('#');
    dfs(adj,res,visited,1,0);
    visited = vb(n+1,false);
    if(check(adj,res,visited,1,0))
    {
        cout<<"YES\n";
        REP(i,0,m)
        {
