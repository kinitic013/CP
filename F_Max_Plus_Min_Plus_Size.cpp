...
void solve(ll t)
{
    ...
    vvi choosen(n,vi(4));
    vvi notChoosen(n,vi(4));
    REP(i,0,n)
    {
        choosen[i] = {INT_MIN,INT_MIN,INT_MAX,0};
        notChoosen[i] = {INT_MIN,INT_MIN,INT_MAX,0};
    }

    choosen[0] = {(2*arr[0])+1,arr[0],arr[0],1};
    ll ans = choosen[0][0];
    REP(i,1,n)
    {
        // not choosen
        if(choosen[i-1][0]>notChoosen[i-1][0])
        {
            notChoosen[i] = choosen[i-1];
        }
        else
        {
            notChoosen[i] = notChoosen[i-1];
        }
        // Choosen
        choosen[i][1] = max(notChoosen[i-1][1],arr[i]);
        choosen[i][2] = min(notChoosen[i-1][2],arr[i]);
        choosen[i][3] = notChoosen[i-1][3] + 1;
        choosen[i][0] = choosen[i][1] + choosen[i][2] + choosen[i][3];

        ans = max(ans,max(choosen[i][0],notChoosen[i][0]));
