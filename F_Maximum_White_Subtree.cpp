...
void dfs_down(vvi& adj,vi& color,vi&down,ll parent,ll node)
{
    ll curr = 0;
    for(auto x : adj[node])
    {
        if(x != parent)
        {
            dfs_down(adj,color,down,node,x);
            curr += max<ll>(0,down[x]);
        }
    }
    if(color[node] == 1)// white
    curr++;
    else
    curr--;
...
void dfs_up(vvi& adj,vi& color,vi&up,vi&down,vi& ans,ll parent,ll node)
{
    ll curr = down[node];
    for(auto x : adj[node])
    {
        ...
        {
            curr -= max<ll>(0,down[x]);
            curr += max<ll>(0,up[node]);

            up[x] = curr;
            ans[x] = down[x] + max<ll>(0,up[x]);
            curr = down[node];
        }
    }
    for(auto x : adj[node])
    {
        if(x != parent)
        {
            dfs_up(adj,color,up,down,ans,node,x);
        }
    }
}
void solve(vi& p)
{
    ...
    vi up(n+1,0),down(n+1,0),ans(n+1,0);
    dfs_down(adj,arr,down,0,1);
    dfs_up(adj,arr,up,down,ans,0,1);
    ans[1] = down[1];
    debug(down,up,ans);
    REP(i,1,n+1)
    cout<<ans[i]<<" ";
