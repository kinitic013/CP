...
void DFS(vvi& adj,vi& distance,vb& marked , ll parent , ll node)
{
    ll dist = LONG_LONG_MIN;
    for(auto x : adj[node])
    {
        if(x != parent)
        {
            DFS(adj,distance,marked,node,x);
            dist = max(dist,distance[x]);
        }
    }
    if(dist == LONG_LONG_MIN)
    {
        if(marked[node])
        {
            distance[node] = max<ll>(0,distance[node]);
        }
    }
    else
    {
...
void DFS_DP(vvi& adj,vi& distance,vi& second_distance , vi& dp , vb& marked , ll parent, ll node)
{
    dp[node] = max<ll>(distance[node] , second_distance[node]);
    pii maxi = {LONG_LONG_MIN,LONG_LONG_MIN};
    for(auto x : adj[node])
    {
        if(x != parent)
        {
            maxi.ss = max<ll>(distance[x],maxi.ss);
            if(maxi.ss > maxi.ff)
    ...
    for(auto x : adj[node])
    {
        if(x!= parent)
        {
            ...
            if(maxi.ff == distance[x])
            {
                second_distance[x] =  max<ll> (second_distance[x], max<ll>(maxi.ss + 2 , second_distance[node] + 1));
            }
            else
            {
                second_distance[x] =  max<ll> (second_distance[x], max<ll>(maxi.ff + 2 , second_distance[node] + 1));
            }
        }
    }
    for(auto x : adj[node])
    {
        if(x != parent)
        {
            DFS_DP(adj,distance,second_distance,dp,marked,node,x);
        }
    }
}
void solve(vi& p)
{
    ...
    vi distance(n+1,LONG_LONG_MIN);
    vi second_distance(n+1,LONG_LONG_MIN);
    vi dp(n+1,LONG_LONG_MIN);
    DFS(adj,distance,marked,0,1);
    debug(distance);
    DFS_DP(adj,distance,second_distance,dp,marked,0,1);
    debug(second_distance,dp,distance);
