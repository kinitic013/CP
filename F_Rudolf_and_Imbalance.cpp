...
void solve(ll t)
{
    ...
    sort(all(difference),greater<ll>());
    ll ans = diff;
    double target = (double)(l + r)/(double)2;
    ll left = 0;
    ll right = k-1;
    while(left < m && right >= 0)
    {
        ll curr = brr[left] + crr[right];
        if(curr > r)
        {
            right--;
        }
