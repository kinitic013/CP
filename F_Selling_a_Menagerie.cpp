...
void solve(ll t)
{
    ...
    REP(i,0,n)
    {
        if(visited[i]==0)
        {
            ...
            if(visited[node] != color)// some other dfs already visited and maked it already 
            {
                color++;
                continue;
            }
            vi cycle;
            cycle.pb(node);
            ll new_node = arr[node];
            while(new_node != node)
            {
                cycle.pb(new_node);
                new_node = arr[new_node];
            }
            ll mini_cost = LONG_LONG_MAX;
            ll mini_idx = -1;
            REP(j,0,cycle.size())
            {
                if(c[cycle[j]]<mini_cost)
                {
                    mini_cost = c[cycle[j]];
                    mini_idx = j;
                }
            }
            mini_idx =( mini_idx + 1)%cycle.size();
            ll idx = (mini_idx+1)%cycle.size();
            ans.pb(cycle[mini_idx]+1);
            ans_visited[cycle[mini_idx]] = 1;
            while(idx != mini_idx)
            {
                ans.pb(cycle[idx]+1);
                ans_visited[cycle[idx]] = 1;
    ...
    REP(i,0,n)
    {
        if(!(ans_visited[arr[i]] || ans_visited[i]))
        {
            adj[i].pb(arr[i]);
            indegree[arr[i]]++;
        }
    ...
    vi ans1 = topoSort(adj,indegree);
    for(auto x : ans1)
    {
        if(ans_visited[x]) continue;
        cout<<x+1<<" ";
    }
    for(auto x : ans )
    cout<<x<<" ";
