...
void solve(ll t)
{
    ...
    // check if array is of this type : 4 5 6 7 1 2 3
    bool flag1 = 1,flag2 = 1;
    ll counter = n-1;
    ll idx = (right_most_maxi - 1 + n)%n;
    while(counter--)
    {
    ...
    if(flag1)
    {
        ans = min<ll>(ans,min<ll>(n-right_most_maxi-1,2+right_most_maxi+1));
    }
    counter = n-1;
    idx = (left_most_maxi + 1 + n)%n;
    while(counter--)
    {
        if(arr[idx] > arr[(idx-1+n)%n])
        {
            flag2 = 0;
            break;
        }
    ...
    if(flag2)
    {
        ans = min<ll>(ans,min<ll>(n-left_most_maxi+1,1+left_most_maxi));
    }
