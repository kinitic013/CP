...
vector<vector<vector<ll>>> dp(201,vector<vector<ll>>(201,vi(201,-1)));
ll query(ll one,ll two,ll three)
{
    ...
    if(one>0)
    {
        ONE = query(one-1,two,three);
    }
    if(two>0)
    {
        TWO = query(one,two-1,three);
    }
    if(three>0)
    {
        THREE = query(one,two,three-1);
    }
    ll curr = 0;
    if(one&1)
    curr = curr^1;
    ...
    dp[one][two][three] = max(ONE,max(TWO,THREE)) + extra;
    return dp[one][two][three];
}
void solve()
{
    ll four,one,two,three;
    cin>>one>>two>>three>>four;
    ll ans = four/2 + query(one,two,three);
    cout<<ans;
}
int main()
{
    ...
    //dp[i][j][k] = maximum games bob could win if there are i 0s , j 1s , k 2s 
    dp[0][0][0] = 0;
    ll t;
    cin >> t;
    while (t--)
