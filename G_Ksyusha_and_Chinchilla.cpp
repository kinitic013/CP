...
void dfs(vector<vpii>& adj,vi& count,vi& edges,ll node,ll parent,ll edge_no)
{
    for(auto x : adj[node])
    {
        if(x.ff != parent)
        {
            dfs(adj,count,edges,x.ff,node,x.ss);
            count[node] += count[x.ff];
        }
    }
    if(count[node] == 3)
    {
