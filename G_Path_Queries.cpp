...
void solve(ll t)
{
    ...
    ll index = 0;
    ll q = 0;
    sort(all(edges),cmpps);
    while(q<m)
    {
        while(index < n-1 && q<m && edges[index][2] <= query[q][0])
        {
            ll u = edges[index][0];
            ll v = edges[index][1];

            ll up = d.findParent(u);
            ll vp = d.findParent(v);

            ll sizeU = d.size[up];
            ll sizeV = d.size[vp];
            // subtract sizeU,sizeV and add sizeU+sizeV

            ans -= (((sizeU*(sizeU-1))/2LL));
            if( vp != up)
            {
                ans -=(((sizeV)*(sizeV-1))/2LL);
            }
            ll total = sizeU+sizeV;
            ans += ((total*(total-1))/2LL);
