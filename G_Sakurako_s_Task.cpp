...
ll count_strictly_less_then_k(vi& arr,ll k)
{
    auto it = upper_bound(all(arr),k-1);
    ll count = it - arr.begin();
    return count;
}
void solve(ll t)
{
    ...
    ll prev_count = 0;
    ll curr_count = count_strictly_less_then_k(arr,k);
    while(prev_count != curr_count)
    {
        ll diff = curr_count - prev_count;
        k += diff;
        prev_count = curr_count;
        curr_count = count_strictly_less_then_k(arr,k);
    }
    cout<<k - 1;
    return;
}
