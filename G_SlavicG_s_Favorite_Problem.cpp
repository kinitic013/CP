...
void DFS(vector<vector<pii>>& adj,vb& visited, vi& arr ,ll node,ll val,ll b)
{
    if(visited[node])
    return;
    visited[node] = 1;
...
void solve(vi& p)
{
    ...
    vi fromA(n+1,INT_MAX);
    vi fromB(n+1,INT_MAX);
    vb visited(n+1,false);
    DFS(adj,visited,fromA,a,0,b);
    visited = vb(n+1,false);
    DFS(adj,visited,fromB,b,0,INT_MAX);
    set<ll> s;
    REP(i,1,n+1)
    {
        if(i != b)
