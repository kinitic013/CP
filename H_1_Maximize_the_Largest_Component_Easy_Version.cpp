...
void solve(vi& p)
{
    ...
    REP(i,0,n)
    {
        REP(j,0,m)
        {
            if(arr[i][j] == '#')
            {
                ...
                if(i-1>=0 && arr[i-1][j] == '#')
                {
                    d.sizeunion(curr,top);
                }
                if(j-1>=0 && arr[i][j-1] == '#')
                {
                    d.sizeunion(curr,left);
                }
            }
        }
    }
    ll ans = *max_element(all(d.size));
    REP(i,0,n)//row
    {
        ll curr = 0;
        map<ll,bool> visited; 
        REP(j,0,m)
        {
            ...
            else
            {
                ...
                if(i-1>=0)// previous row
                {
                    if(arr[i-1][j] == '#')
                    {
                        ll parent = d.findParent((i-1)*m + j);
                        if(!visited[parent])
                        {
                            visited[parent] = 1;
                            curr += d.size[parent];
    ...
    REP(j,0,m)//column
    {
        ...
        REP(i,0,n)
        {
            ...
            {
                curr++;
                if(j-1>=0)// previous column
                {
                    if(arr[i][j-1] == '#')
                    {
                        ll parent = d.findParent((i)*m + j-1);
                        if(!visited[parent])
                        {
                            visited[parent] = 1;
                            curr += d.size[parent];
