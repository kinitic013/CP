...
pii DFS(vvi& adj,vi& distance,ll parent,ll node)
{
    if(distance[node] != -1)
    {
    ...
    for(auto x : adj[node])
    {
        if(x != parent)
        {
            pii curr = DFS(adj,distance,node,x);
            if(curr.ff != -1)
            {
                return curr;
            }
...
ll dist(vvi& adj,vb& visited , ll parent , ll node , ll target)
{
    if(node == target)
    return 0;
    ...
    for(auto x : adj[node])
    {
        if(!visited[x])
        {
            ll curr =  dist(adj,visited,node,x,target);
            ans = min(ans , curr);
        }
    }
    return ans + 1;
...
void solve(vi& p)
{
    ...
    vi distanceA(n+1,-1);
    vi distanceB(n+1,-1);
    pii valueA = DFS(adj,distanceA,0,a);
    pii valueB = DFS(adj,distanceB,0,b);
    debug(valueA,valueB);
    vb visited(n+1,false);
    ll distanceBetween = dist(adj,visited,0,a,valueB.ff);
    if(distanceBetween <= valueB.ss )
    {
        cout<<"NO";
    }
    else
