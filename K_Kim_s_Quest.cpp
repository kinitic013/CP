...
void solve(ll t)
{
    ...
    */ 
    ...
    REP(i,1,n)
    {
        ...
        if(arr[i]&1)
        {
            dp[i][6] = (((dp[i][6] + dp[i-1][2])%MOD + dp[i-1][3])%MOD)%MOD;
            dp[i][4] = ((dp[i][4] + dp[i-1][0])%MOD+dp[i-1][3])%MOD; //done
            dp[i][3] = (((dp[i][3] + dp[i-1][1])%MOD + dp[i-1][2])%MOD )%MOD;
            dp[i][0]++;
        }
        else
        {
            dp[i][6] = ((dp[i][6]+ dp[i-1][5])%MOD + dp[i-1][4])%MOD;
            dp[i][5] = (((2*dp[i][5])%MOD + dp[i-1][1])%MOD)%MOD;
            dp[i][2] = (((dp[i][2] + dp[i-1][0])%MOD)%MOD + dp[i-1][4])%MOD;
            dp[i][1]++;
        }
        debug(dp[i]);
        cout.flush();
