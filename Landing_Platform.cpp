...
bool isValid(pii& a, ll n,ll m)
{
    if((a.ff<= n && a.ff>=1) && (a.ss<=m && a.ss>=1))
    {
        return 1;
    }
    return 0;
...
void solve(ll t)
{
    ...
    vvi main_diagonal(n+2,vi(m+2,0));
    REP(i,1,n+1)
    {
        REP(j,1,m+1)
        {
            main_diagonal[i][j] = arr[i][j] + main_diagonal[i-1][j-1];
        }
    }
    vvi other_diagonal(n+2,vi(m+2,0));
    REP(j,1,m+1)
    {
        REP(i,1,n+1)
        {
            other_diagonal[i][j] = arr[i][j] + other_diagonal[i+1][j-1];
        }
    }
    debug(main_diagonal);
    debug(other_diagonal);
        ...
        REP(j,1,m+1)
        {
            REP(size,2,n+1)
            {
                if(size&1)
                {
                    // i,j is center
                    // ohter
                    ll other = LONG_LONG_MIN;
                    ll h = size/2;
                    pii A = {i - h , j + h};
                    pii B = {i + h , j - h};
                    if(isValid(A,n,m) && isValid(B,n,m))
                    {
                        other = other_diagonal[A.ff][A.ss] - other_diagonal[B.ff+1][B.ss-1];
                    }

                    // main
                    ll main = LONG_LONG_MIN;
                    pii C = {i + h , j + h};
                    pii D = {i - h , j - h};
                    if(isValid(C,n,m) && isValid(D,n,m))
                    {
                        main = main_diagonal[C.ff][C.ss] - main_diagonal[D.ff-1][D.ss-1] ;
                    }
                    
                    if(other == LONG_LONG_MIN || main == LONG_LONG_MIN)
                    continue;
                    else
                    {
                        debug(res,other+main-arr[i][j]);
                        res = max<ll>(res,other + main - arr[i][j]);
                    }
                }
                else
                {
                    ...
                    {
                        debug(res,other+main);
                        res = max<ll>(res,other + main );
                    }
                }
            }
        }
