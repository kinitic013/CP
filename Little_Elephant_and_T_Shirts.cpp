...
void solve(ll t)
{
    ...
    REP(i,1,n+1)
    {
        ...
        // Use a stringstream to parse the input line
    ...
    REP(i,1,101)// 100
    {
        REP(j,0,arr[i].size()) // 10
        {
            for(int k = (1<<n)-1 ; k>=0; k--)
            {
                ll curr_state = k;
                if(!((k & (1 << (arr[i][j]-1)))) && dp[i-1][k]!=0)
                {
                    ll next_state = k |(1 << (arr[i][j]-1));
                    dp[i][k | (1 << (arr[i][j]-1))] = (dp[i-1][k] +  dp[i][k | (1 << (arr[i][j]-1))] )%MOD;
                }
            }
        }
        for(int k = 0;k<(1<<n);k++)
