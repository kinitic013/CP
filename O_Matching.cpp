...
void solve(ll t)
{
    ...
    REP(mask,0,limit-1)
    {
        ll bit_count = __builtin_popcount(mask);
        REP(i,0,n)
        {
            if(arr[bit_count][i] == 1 && !(mask & (1<<i)))// matching is possible between ith women and some size men  also women is not already choosen
            {
