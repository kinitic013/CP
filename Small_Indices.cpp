...
void solve(ll t)
{
    ...
    vvi dp(n,vi(4*n+1,-1));
    vvi m(n,vi(4*n+1,INT_MIN));

    dp[][a[0]+b[0]] = 0;
    m[a[0]+b[0]] = max(a[0],b[0]);

    dp[a[0]+b[1]] = 0;
    m[a[0] + b[1]] = max(a[0],b[1]);

    dp[a[1] + b[0]] = 0;
    m[a[1] + b[0]] = max(a[1],b[0]);

    dp[a[1] + b[1]] = 0;
    m[a[1] + b[1]] = max(a[1],b[1]);
    ...
    REP(i,2,n)
    {
        for(int j =4*n ;j>=0;j--)
        {
            ...
            {
                // if a[i] is choosen
                ll next_idx_1 = max<ll>(j,m[j] + a[i]);
                ll next_val_1 = dp[j] + (j>=a[i]);


                

                // if b[i] is choosen
                ll next_idx_2 = max<ll>(j,m[j] + b[i]);
                ll next_val_2 = dp[j] + (j>=b[i]);
                
                if(dp[next_idx_1] <= next_val_1)
                {
                    dp[next_idx_1] = next_val_1;
                    m[next_idx_1] = max<ll>(m[next_idx_1],a[i]);
                }
                if(dp[next_idx_2] <= next_val_2)
                {
                    dp[next_idx_2] = next_val_2;
                    m[next_idx_2] = max<ll>(m[next_idx_2],b[i]);
                }
            }
        }
    }
