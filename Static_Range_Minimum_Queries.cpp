...
// ll query(ll l, ll r,ll isIdempotent=true)
// {
//     ll size = r - l + 1;
//     ll range = log2(size);
//     if(isIdempotent)
//     {
//         return min(sparse[l][range],sparse[r - (1<<range) +1][range]);
//     }
//     else
//     {
//         // No need to use Sparse Table use SegTree
//     }
//     return 0;
// }
// void solve(ll t)
...
//         REP(i,0,n)
//         {
//             sparse[i].resize(20,LONG_LONG_MAX);
//         }
//         REP(i,0,n)
//         {
//             sparse[i][0] = arr[i];
...
//                 if(i + (1<<(j-1))<n)
//                 {
//                     ll curr_val = min(sparse[i][j-1],sparse[i+(1<<(j-1))][j-1]);
//                     sparse[i][j] = curr_val;
//                 }
//             }
//         }
...
//     return;
// }
ll query(vi& arr , vi& lookup ,const ll B , ll n , ll l , ll r )
{
    ...
    while(i <= r)
    {
        ll idx = i/B;
        ll next_idx = idx+1;
        ll i_limit = next_idx*B;
        if(i == idx*B && r > i_limit)
        {
            res = min(res , lookup[idx]);
            i += B;
            continue;
        }
        else
...
void solve(ll t)
{
    ...
    REP(i,0,n) cin>>arr[i];
    const ll B = sqrt(n);
    vi lookup((n/B) + 1 , LONG_LONG_MAX);
    REP(i,0,n)
    {
        ll idx = i/B;
        lookup[idx] = min(lookup[idx] , arr[i]);
    }
    debug(lookup);
    REP(i,0,q)
    {
