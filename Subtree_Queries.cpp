...
ll query(vi& arr,ll start,ll end , ll index , ll l , ll r)
{
    ll left = index*2 + 1;
    ll right = index*2 + 2;
    ll mid = (start + end )/2;
    ...
    {
        ll leftValue = query(arr,start,mid,left,l,r);
        ll rightValue = query(arr,mid+1,end,right,l,r);
...
void compute(vvi& adj)
{
    ...
    debug(flat,level,first_occurence,range);
}
void solve(ll t)
{
    ll n,q;cin>>n>>q;
    ...
    REP(i,0,q)
    {
        ...
        if(type == 1)
        {
            ...
            update(v,0,m-1,0,range[pos].ff,val);
            update(v,0,m-1,0,range[pos].ss,val);
        }
        else
        {
            ll pos;cin>>pos;
            cout<<query(v,0,m-1,0,range[pos].ff,range[pos].ss)/2<<endl;
        }
    }
    return;
}
