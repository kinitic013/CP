...
int dfs_two(vector<string>& arr,vector<vector<int>> & state,int& n,int& m,int i,int j,int prev_i,int prev_j)
{
    if(i>=n || i<0 || j>=m || j<0)
    {
        return 0;
    ...
    if(state[i][j] == -1)
    {
        ...
        if(next_i>=n || next_i<0 || next_j>=m || next_j<0)
        {
            state[i][j] = 0;
        }
        else
        state[i][j] = dfs_two(arr,state,n,m,next_i,next_j,i,j);
    }
    return state[i][j];
}
bool dfs(vector<string>& arr,vector<vector<int>> & state,int& n,int& m,int i,int j,int prev_i,int prev_j,ll& count)
{
    if(i>=n || i<0 || j>=m || j<0)
    {
        count = 0;
    ...
    if(state[i][j] == -1)
    {
        ...
        else
        {
            ...
            {
                count++;
                state[i][j] = dfs(arr,state,n,m,next_i,next_j,i,j,count);
            }
            else
            {
                state[i][j] = state[next_i][next_j];
            }
            if(state[i][j] == 0)
            {
                count = 0;
...
void solve(ll t)
{
    ...
    REP(i,0,n)
    {
        ...
        {
            if(state[i][j] == -1)
            state[i][j] = dfs_two(arr,state,n,m,i,j,-1,-1);
        }
    }
    REP(i,0,n)
    {
        REP(j,0,m)
        {
            if(arr[i][j] == '?')
            {
                if(i-1>=0 && state[i-1][j])// looking up
                {
                    arr[i][j] = 'U';
                }
                else if(i+1<n && state[i+1][j])// looking down
                {
                    arr[i][j] = 'D';
                }
                else if(j+1<m && state[i][j+1]) //lookgin right
                {
                    arr[i][j] = 'R';
                }
                else if(j-1>=0 && state[i][j-1]) // looking left
                {
                    arr[i][j] = 'L';
                }
                else
    ...
    REP(i,0,n)
    {
        REP(j,0,m)
        {
            ...
            if(state[i][j] == -1)
            {
                dfs(arr,state,n,m,i,j,-1,-1,count);
                ans += count;
            }
        }
    }
