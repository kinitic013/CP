...
vi level;
vi first_occurence;
void dfs(vvi& adj,vi& flat,vi& level , ll curr_level , ll node , ll parent)
{
    flat.push_back(node);
    ...
    for(auto& x : adj[node])
    {
        if(x != parent)
        {
            dfs(adj,flat,level,curr_level+1,x,node);
            flat.push_back(node);
        }
    }
}
void build(vector<ll>& arr,vi& level, ll start , ll end , ll index)
{
    if(start == end)
    {
        segTree[index] = start;
    ...
    ll left = index*2+1;
    ll right = index*2 + 2;
    build(arr,level,start,mid,left);
    build(arr,level,mid+1,end,right);
    if(level[arr[segTree[left]]] < level[arr[segTree[right]] ])
    {
        segTree[index] = segTree[left];
    }
    else
    {
        segTree[index] = segTree[right];
    }
    // segTree[index] = min(segTree[left],segTree[right]);
}
ll query(vi& arr,vi& level,ll start,ll end , ll index , ll l , ll r)
{
    ll left = index*2 + 1;
    ll right = index*2 + 2;
    ll mid = (start + end )/2;
    ...
    {
        ll leftValue = query(arr,level,start,mid,left,l,r);
        ll rightValue = query(arr,level,mid+1,end,right,l,r);
        if(leftValue == INT_MAX)
        return rightValue;
        if(rightValue == INT_MAX)
        return leftValue;

        if(level[arr[leftValue]] < level[arr[rightValue]])
        return leftValue;
        else
        return rightValue;
    }
...
void compute(vvi& adj)
{
    int n = adj.size()-1;
    level.resize(n+1,0); level[0] = INT_MAX;
    first_occurence.resize(n+1,0);
    dfs(adj,flat,level,0,1,0);
    for(int i=flat.size()-1;i>=0;i--)
    first_occurence[flat[i]] = i;
    
    debug(flat,level,first_occurence);

    ll m = flat.size();
    segTree.resize(4*m);
    build(flat,level,0,m-1,0);
}
void solve(ll t)
{
    ll q,n;cin>>n>>q;
    ...
    {
        ll a,b;cin>>a>>b;
        ll idx = query(flat,level,0,m-1,0,min(first_occurence[a],first_occurence[b]),max(first_occurence[a],first_occurence[b]));
        // cout<<flat[idx]<<"\n";

        ll parent = flat[idx];
        ll res = abs(level[parent] - level[a]) + abs(level[parent] - level[b]);
        cout<<res<<"\n";
