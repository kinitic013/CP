...
void solve(ll t)
{
    ...
    if(n == 2)
    {
        cout<<findCommon(arr[0],arr[1]);
        return;
    }
    vvi collision_count(n+3,vi(2,0));
    REP(i,1,n)
    {
        if(i-1>=0)
        collision_count[i][0] = findCommon(arr[i],arr[i-1]);
        if(i-2>=0)
        collision_count[i][1] = findCommon(arr[i],arr[i-2]);
    }
    ll res = INT_MAX;
    vector<vector<vi>> dp(n,vvi(2,vi(2,INT_MAX)));
    dp[0][1][1] = 0;
    dp[0][1][0] = 0;
    REP(i,1,n)
    {
        ...
        {
            ll val1 = dp[i-1][0][0];
            ll val2 = dp[i-1][0][1] + collision_count[i][1];
            dp[i][0][0] = min<ll>(val1,val2);
        }
        // Left Right
        {
            ll val1 = dp[i-1][1][0] ;
            ll val2 = dp[i-1][1][1] + collision_count[i][1];
            dp[i][0][1] = min<ll>(val1,val2) + collision_count[i][0];
        }
        // Rigth Left
        {
            dp[i][1][0] = min<ll>(dp[i-1][0][0],dp[i-1][0][1]);
        }
        // Right Right
        {
            dp[i][1][1] = min<ll>(dp[i-1][1][1],dp[i-1][1][0]);
        }
    }
