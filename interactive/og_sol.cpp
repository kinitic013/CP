...
void solve(ll t)
{
    ...
    while(state[pointer])
    pointer++;
    while(leaf.size() > 1 && pointer<n)
    {
        while(state[pointer])
        pointer++;

        ll curr_leaf = *(leaf.begin());
        leaf.erase(leaf.begin());
        {
            cout<<"? "<<curr_leaf<<" "<<pointer<<endl;
            cout.flush();
        }
        {
            ll r;cin>>r;
